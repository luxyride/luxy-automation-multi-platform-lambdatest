...
public class DEV_TC_1542_VerifyAddressSuggestionListWhenSearchForHotelsMotelsInPickupOffExtraStopAddressFieldsTraveAgent_Main
		extends TestBase {
	...
	@FindBy(xpath = "//*[(@title='SUV') and normalize-space()='SUV']")
	WebElement bookSUV;
	...
	public DEV_TC_1542_VerifyAddressSuggestionListWhenSearchForHotelsMotelsInPickupOffExtraStopAddressFieldsTraveAgent_Main(
			WebDriver driver) {
		try {
			this.driver = driver;
	...
	public Boolean addFromAddress(Boolean visibilityStatus) {
		try {
			action = new Actions(driver);
			fromAddress.click();
			objTestBase.defaultWaitTime(1000);
			...
			objTestBase.defaultWaitTime(1000);
			fromAddress.sendKeys(prop.getProperty("fromAddressAirport"));
			objTestBase.defaultWaitTime(6000);
			action.sendKeys(Keys.DOWN).build().perform();
			objTestBase.defaultWaitTime(1000);
			action.sendKeys(Keys.ENTER).build().perform();
			objTestBase.defaultWaitTime(2000);
			expected = fromAddress.getAttribute("value");
			if (expected != null)
				visibilityStatus = true;
			else
				visibilityStatus = false;
	...
	public Boolean addToAddress(Boolean visibilityStatus) {
		try {
			action = new Actions(driver);
			toAddress.sendKeys(prop.getProperty("toAddressHotel"));
			objTestBase.defaultWaitTime(6000);
			action.sendKeys(Keys.DOWN).build().perform();
			objTestBase.defaultWaitTime(1000);
			action.sendKeys(Keys.ENTER).build().perform();
			objTestBase.defaultWaitTime(2000);
			expected = toAddress.getAttribute("value");
			if (expected != null)
				visibilityStatus = true;
			else
				visibilityStatus = false;
	...
	public Boolean addExtraStop(Boolean visibilityStatus) {
		try {
			action = new Actions(driver);
			addextraStop.sendKeys(prop.getProperty("extraStopMotel"));
			objTestBase.defaultWaitTime(6000);
			action.sendKeys(Keys.DOWN).build().perform();
			objTestBase.defaultWaitTime(1000);
			action.sendKeys(Keys.ENTER).build().perform();
		...
		try {
			...
			if (expected.toLowerCase().contains("all")) {
				...
				if (bookingRideTripId.isDisplayed()
					...
					utillLogger.info(
							"DEV_TC_1542_VerifyAddressSuggestionListWhenSearchForHotelsMotelsInPickupOffExtraStopAddressFieldsTraveAgent_Main - Capture the Trip ID = "
									+ tripID);
					visibilityStatus = true;
				} else
					visibilityStatus = false;
