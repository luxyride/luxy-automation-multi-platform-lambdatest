...
public class DEV_TC_1316_VerifyRideBookingWhenZoneTypeAsAirport_TravelAgent_Main extends TestBase {
	...
	public DEV_TC_1316_VerifyRideBookingWhenZoneTypeAsAirport_TravelAgent_Main(WebDriver driver) {
		try {
			this.driver = driver;
	...
	public void addToAddress(String scenario) {
		try {
			...
			if (scenario == "lessthanConfigMiles")
				toAddress.sendKeys(prop.getProperty("zoneTypeAirportToAddressNegative"));
			else if (scenario == "greaterthanConfigMilesNegative")
				toAddress.sendKeys(prop.getProperty("zoneTypeAirportToAddressNegative"));
			else if (scenario == "positivescenario")
				toAddress.sendKeys(prop.getProperty("zoneTypeAirportToAddressPositive"));
			objTestBase.defaultWaitTime(3000);
			action.sendKeys(Keys.DOWN).build().perform();
			objTestBase.defaultWaitTime(1000);
			action.sendKeys(Keys.ENTER).build().perform();
	...
	public Boolean verifyVisibilityOfWarningMsg(Boolean visibilityStatus) {
		try {
			...
			if (displayStatus.isDisplayed()) {
				...
				visibilityStatus = false;
			objTestBase.defaultWaitTime(1000);
			if (visibilityStatus == true && msgText.contains(prop.getProperty("zoneTypeAirportConfigTime"))
					&& msgText.contains(prop.getProperty("zoneTypeAirportConfigMiles")))
				visibilityStatus = true;
			else
				visibilityStatus = false;
			System.out.println(msgText);
	...
	public void addDispatchFromAddress() {
		try {
			...
			objTestBase.defaultWaitTime(1000);
			taFromAddress.sendKeys(prop.getProperty("zoneTypeAirportFromAddress"));
			objTestBase.defaultWaitTime(3000);
			action.sendKeys(Keys.DOWN).build().perform();
			objTestBase.defaultWaitTime(1000);
			action.sendKeys(Keys.ENTER).build().perform();
	...
	public void addDispatchToAddress() {
		try {
			action = new Actions(driver);
			taToAddress.sendKeys(prop.getProperty("zoneTypeAirportToAddressPositive"));
			objTestBase.defaultWaitTime(3000);
			action.sendKeys(Keys.DOWN).build().perform();
			objTestBase.defaultWaitTime(1000);
			action.sendKeys(Keys.ENTER).build().perform();
