...
public class DEV_TC_1779_HourlyVerifyBookingARideInHourlyTabTAByEnablingSameAsPickUpAddressCheckBox_Main extends TestBase {
	...
	WebElement bookingRidetravelAgenthourlyRideTripID;
	...
	public DEV_TC_1779_HourlyVerifyBookingARideInHourlyTabTAByEnablingSameAsPickUpAddressCheckBox_Main(WebDriver driver) {
		try {
			this.driver = driver;
	...
	public String captureRideBookingIDs(Boolean visibilityStatus, String scenario, String travelAgenthourlyRideTripID) {
		try {
			action = new Actions(driver);
			ridesTabAll.click();
			...
			if (expected.toLowerCase().contains("all")) {
				...
				if (bookingRidetravelAgenthourlyRideTripID.isDisplayed()
						&& !rideBookingIdsTravelAgent.containsValue(bookingRidetravelAgenthourlyRideTripID.getText())) {
					...
					System.out.println(bookingRidetravelAgenthourlyRideTripID.getText());
							...
							&& ridePlaceDateTime.getText().toLowerCase().contains(currDate))// 06-27-2023 12:00 AM
					{
						...
						rideBookingIdsConcierge.put(vehicleType.getText().toLowerCase(),
								bookingRidetravelAgenthourlyRideTripID.getText().toString());
						objTestBase.defaultWaitTime(500);
						travelAgenthourlyRideTripID = bookingRidetravelAgenthourlyRideTripID.getText().toString();
						objTestBase.defaultWaitTime(1000);
					}
					utillLogger.info(
							"DEV_TC_1152_VerifyNewBookingFunctionalityInTravelAgentSite_Main - Capture the Trip ID = "
									+ travelAgenthourlyRideTripID);
					visibilityStatus = true;
				} else
					visibilityStatus = false;
			} else
		...
		return travelAgenthourlyRideTripID;
	}
