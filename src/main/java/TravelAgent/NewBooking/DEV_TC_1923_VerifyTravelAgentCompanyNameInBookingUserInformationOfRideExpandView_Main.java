...
public class DEV_TC_1923_VerifyTravelAgentCompanyNameInBookingUserInformationOfRideExpandView_Main extends TestBase {
	...
	WebElement dispatchBookingUserInfo;

	@FindBy(xpath = "//b[contains(normalize-space(),'Travel Arranger Company Name')]")
	WebElement dispatchTravelAgentCompnayName;
	...
	public DEV_TC_1923_VerifyTravelAgentCompanyNameInBookingUserInformationOfRideExpandView_Main(WebDriver driver) {
		try {
			this.driver = driver;
	...
	public Boolean verifyTACompanyNameExpandView(Boolean visibilityStatus) {
		try {
			action = new Actions(driver);
			action.moveToElement(dispatchTripID).click().build().perform();
			...
			if (dispatchTravelAgentCompnayName.isDisplayed()) {
				String tempVal = dispatchTravelAgentCompnayName.getText();
				if (tempVal != null)
					visibilityStatus = true;
				else
					visibilityStatus = false;
