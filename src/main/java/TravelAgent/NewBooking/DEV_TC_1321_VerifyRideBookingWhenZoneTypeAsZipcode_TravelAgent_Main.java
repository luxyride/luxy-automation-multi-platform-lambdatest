...
public class DEV_TC_1321_VerifyRideBookingWhenZoneTypeAsZipcode_TravelAgent_Main extends TestBase {
	...
	WebElement taupdateBtn;

	@FindBy(xpath = "(//span[normalize-space()='Rides'])[1]")
	WebElement ridesSection;

	@FindBy(xpath = "//button[normalize-space()='Sign In']")
	WebElement tasignInBtn_Login;

	@FindBy(xpath = "//input[@placeholder='Pickup Address']")
	WebElement taFromAddress;

	@FindBy(xpath = "//input[@placeholder='Drop Off Address']")
	WebElement taToAddress;

	@FindBy(xpath = "//span[@class='MuiIconButton-label']//div")
	WebElement tacalendar;

	@FindBy(xpath = "//input[@id='date-picker-inline']")
	WebElement tadateInput;

	@FindBy(xpath = "//button[normalize-space()='Get Quote']")
	WebElement tagetQuote;

	@FindBy(xpath = "//span[normalize-space()='Choose Vehicle']//following-sibling::div")
	List<WebElement> tavechileAvailableSection;

	@FindBy(xpath = "//a[normalize-space()='Edit']")
	WebElement taEDITVehicle;

	@FindBy(xpath = "(//button[@class='MuiButtonBase-root MuiIconButton-root MuiPickersCalendarHeader-iconButton']//span)[1]")
	WebElement tanextMonthArrow;
	...
	WebElement taalertMessge;

	@FindBy(xpath = "(//input[@type='text'])[1]")
	WebElement taSKUNumber;

	@FindBy(xpath = "(//span[normalize-space()='Pricing'])[1]")
	WebElement tapricingSection;

	@FindBy(xpath = "(//span[normalize-space()='Service Rate'])[1]")
	WebElement taserviceRateTab;
	...
	@FindBy(xpath = "//input[@type='text']")
	WebElement eMailtaInput;

	@FindBy(xpath = "//input[@type='password']")
	WebElement passwordtaInput;

	@FindBy(xpath = "//span[contains(text(),'This trip can not be quoted at the moment')]")
	List<WebElement> tawarningMsg;
	...
	public DEV_TC_1321_VerifyRideBookingWhenZoneTypeAsZipcode_TravelAgent_Main(WebDriver driver) {
		try {
			this.driver = driver;
	...
	public void addFromAddress() {
		try {
			...
			objTestBase.defaultWaitTime(2000);
			fromAddress.sendKeys(prop.getProperty("zoneTypeZipcodeFromAddress"));
			objTestBase.defaultWaitTime(3000);
	...
	public void addToAddress(String scenario) {
		try {
			...
			if (scenario == "lessthanConfigMiles")
				toAddress.sendKeys(prop.getProperty("zoneTypeZipcodeToAddressNegative"));
			else if (scenario == "greaterthanConfigMilesNegative")
				toAddress.sendKeys(prop.getProperty("zoneTypeZipcodeToAddressNegative"));
			else if (scenario == "positivescenario")
				toAddress.sendKeys(prop.getProperty("zoneTypeZipcodeToAddressPositive"));
			objTestBase.defaultWaitTime(3000);
			action.sendKeys(Keys.DOWN).build().perform();
			objTestBase.defaultWaitTime(1000);
			action.sendKeys(Keys.ENTER).build().perform();
	...
	public String captureRideBookingIDs(Boolean visibilityStatus, String scenario, String tripID) {
		try {
			...
			if (expected.toLowerCase().contains("all")) {
				...
				if (taTripID.isDisplayed() && !rideBookingIdsTravelAgent.containsValue(taTripID.getText())) {
					...
					System.out.println(taTripID.getText());
					...
					{
						objTestBase.defaultWaitTime(1000);
						tripID = taTripID.getText().toString();
						objTestBase.defaultWaitTime(1000);
						testStatus = "PASSED";
					} else
						testStatus = "FAILED";
	...
	public Boolean verifyVisibilityOfWarningMsg(Boolean visibilityStatus) {
		try {
			...
			if (displayStatus.isDisplayed()) {
				...
				visibilityStatus = false;
			objTestBase.defaultWaitTime(1000);
			if (visibilityStatus == true && msgText.contains(prop.getProperty("zoneTypeZipcodeConfigTime"))
					&& msgText.contains(prop.getProperty("zoneTypeZipcodeConfigMiles")))
				visibilityStatus = true;
			else
				visibilityStatus = false;
			System.out.println(msgText);
	...
	public void eMailDispatchInput() {
		try {
			action = new Actions(driver);
			eMailtaInput.sendKeys(prop.getProperty("dispatchadminMail"));
			action.sendKeys(Keys.TAB).build().perform();
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public void passwordDispatchInput() {
		try {
			action = new Actions(driver);
			passwordtaInput.sendKeys(prop.getProperty("dispatchadminPwd"));
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	}
	...
	public String captureSKUNumber(String skuNumber) {
		try {
			skuNumber = taSKUNumber.getAttribute("value");
			objTestBase.defaultWaitTime(1000);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public void addDispatchFromAddress() {
		try {
			...
			taFromAddress.click();
			objTestBase.defaultWaitTime(1000);
			taFromAddress.sendKeys(Keys.CONTROL + "A");
			taFromAddress.sendKeys(Keys.DELETE);
			objTestBase.defaultWaitTime(1000);
			taFromAddress.sendKeys(prop.getProperty("zoneTypeZipcodeFromAddress"));
			objTestBase.defaultWaitTime(3000);
			action.sendKeys(Keys.DOWN).build().perform();
			objTestBase.defaultWaitTime(1000);
			action.sendKeys(Keys.ENTER).build().perform();
	...
	public void addDispatchToAddress() {
		try {
			action = new Actions(driver);
			taToAddress.sendKeys(prop.getProperty("zoneTypeZipcodeToAddressPositive"));
			objTestBase.defaultWaitTime(3000);
			action.sendKeys(Keys.DOWN).build().perform();
			objTestBase.defaultWaitTime(1000);
			action.sendKeys(Keys.ENTER).build().perform();
	...
	public void dipatchclickOngetQuote() {
		try {
			action = new Actions(driver);
			action.moveToElement(tagetQuote).click().build().perform();
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public Boolean visibilityOfVechileSection(Boolean visibilityStatus) {
		try {
			objTestBase.defaultWaitTime(2000);
			if (tavechileAvailableSection.size() != 0)
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception e) {
	...
	public void dispatchclickonEDITVehicle() {
		try {
			action = new Actions(driver);
			action.moveToElement(taEDITVehicle).click().build().perform();
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void dispatchenterDate() {
		try {
			action = new Actions(driver);
			objTestBase.defaultWaitTime(2000);
			action.moveToElement(tacalendar).click().build().perform();
			objTestBase.defaultWaitTime(2000);
	...
	public void dipatchclickOnUpdateButton() {
		try {
			action = new Actions(driver);
			action.moveToElement(taupdateBtn).click().build().perform();
			objTestBase.defaultWaitTime(5000);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public Boolean verifyDispatchDoNotServeUpdateConfirmation(Boolean visibilityStatus) {
		try {
			WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(20));
			WebElement displayStatus = wait.until(ExpectedConditions.visibilityOf(taalertMessge));
			if (displayStatus.isDisplayed())
				visibilityStatus = true;
			else
				visibilityStatus = false;
	...
	public Boolean verifyVisibilityOfDoNotServeWarningMsg(Boolean visibilityStatus) {
		try {
			objTestBase.defaultWaitTime(3000);
			if (tawarningMsg.size() != 0) {
				waitTimeForElement(warningMsg.get(0));
				if (tawarningMsg.get(0).getText().toLowerCase().contains("this trip can not be quoted at the moment"))
					visibilityStatus = true;
				else
					visibilityStatus = false;
	...
	public void dispatchclickPricingSection() {
		try {
			action = new Actions(driver);
			action.moveToElement(tapricingSection).click().build().perform();
			objTestBase.defaultWaitTime(1000);
			action.sendKeys(Keys.TAB).build().perform();
			action.sendKeys(Keys.TAB).build().perform();
		} catch (Exception ex) {
	...
	public Boolean dipatchclickOnEDITButton(Boolean visibilityStatus) {
		try {
			...
			wait.until(ExpectedConditions.visibilityOf(taEDIT));
			action = new Actions(driver);
			action.moveToElement(taEDIT).click().build().perform();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return visibilityStatus;
	...
	public Boolean dispatchVisibilityOfSigninButton(Boolean visibilityStatus) {
		try {
			if (tasignInBtn_Login.isDisplayed())
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception e) {
