...
public class DEV_TC_1406_VerifyDisplayingOfInternalNotesAfterBookingRideInTravelAgentPortal_Main extends TestBase {
	...
	@FindBy(xpath = "//*[@name='internalNotes']")
	...
	@FindBy(xpath = "(//span[normalize-space()='Pay Now'])[1]")
	...
	public DEV_TC_1406_VerifyDisplayingOfInternalNotesAfterBookingRideInTravelAgentPortal_Main(WebDriver driver) {
		try {
			this.driver = driver;
	...
	public Boolean searchCorporate(Boolean visibilityStatus) {
		try {
			action = new Actions(driver);
			objTestBase.defaultWaitTime(1000);
			action.moveToElement(corpToggleBtn.get(0)).click().build().perform();
	...
	public void enterInternalNotes() {
		try {
			internalNotes.sendKeys(prop.getProperty("internalNotes"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void updateInternalNotes() {
		try {
			...
			internalNotes.click();
			objTestBase.defaultWaitTime(1000);
			internalNotes.sendKeys(Keys.CONTROL + "A");
			internalNotes.sendKeys(Keys.DELETE);
			objTestBase.defaultWaitTime(1000);
			internalNotes.sendKeys("Updated " + prop.getProperty("internalNotes"));
			objTestBase.defaultWaitTime(3000);
			action.sendKeys(Keys.DOWN).build().perform();
			objTestBase.defaultWaitTime(1000);
			action.sendKeys(Keys.ENTER).build().perform();
			...
			if (expected.toLowerCase().contains("all")) {
				...
				if (bookingRideTripId.isDisplayed()
					...
					if ((!rideStatus.getText().toLowerCase().contains("cancelled")
							...
							&& bookedPassengerName.getText().toLowerCase().contains("qa")
	...
	public String verifyInternalNotesValueBeforeUpdate(Boolean visibilityStatus, String internalNotesBeforeUpdate) {
		try {
			internalNotesBeforeUpdate = internalNotes.getText();
			if (internalNotesBeforeUpdate != null)
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception e) {
		...
		return internalNotesBeforeUpdate;
	}

	public Boolean verifyInternalNotesValueAfterUpdate(Boolean visibilityStatus, String internalNotesBeforeUpdate,
			String internalNotesAfterUpdate) {
		try {
			internalNotesAfterUpdate = internalNotes.getText();
			if ((internalNotesBeforeUpdate != internalNotesAfterUpdate) || (!internalNotesAfterUpdate.contains(",")))
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception e) {
	...
	public void clickCorporateToggle() {
		try {
			action = new Actions(driver);
			action.moveToElement(corpToggleBtn.get(0)).click().build().perform();
		} catch (Exception ex) {
