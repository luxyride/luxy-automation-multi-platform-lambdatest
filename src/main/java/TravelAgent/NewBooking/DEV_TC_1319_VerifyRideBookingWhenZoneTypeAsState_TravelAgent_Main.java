...
public class DEV_TC_1319_VerifyRideBookingWhenZoneTypeAsState_TravelAgent_Main extends TestBase {
	...
	public DEV_TC_1319_VerifyRideBookingWhenZoneTypeAsState_TravelAgent_Main(WebDriver driver) {
		try {
			this.driver = driver;
	...
	public Boolean verifyVisibilityOfWarningMsg(Boolean visibilityStatus) {
		try {
			...
			if (displayStatus.isDisplayed()) {
				...
				visibilityStatus = false;
			objTestBase.defaultWaitTime(1000);
			if (visibilityStatus == true && msgText.contains(prop.getProperty("zoneTypeStateConfigTime"))
					&& msgText.contains(prop.getProperty("zoneTypeStateConfigMiles")))
				visibilityStatus = true;
			else
				visibilityStatus = false;
			System.out.println(msgText);
	...
	public void addDispatchFromAddress() {
		try {
			...
			objTestBase.defaultWaitTime(1000);
			taFromAddress.sendKeys(prop.getProperty("zoneTypeStateFromAddress"));
			objTestBase.defaultWaitTime(3000);
			action.sendKeys(Keys.DOWN).build().perform();
			objTestBase.defaultWaitTime(1000);
			action.sendKeys(Keys.ENTER).build().perform();
	...
	public void addDispatchToAddress() {
		try {
			action = new Actions(driver);
			taToAddress.sendKeys(prop.getProperty("zoneTypeStateToAddressPositive"));
			objTestBase.defaultWaitTime(3000);
			action.sendKeys(Keys.DOWN).build().perform();
			objTestBase.defaultWaitTime(1000);
			action.sendKeys(Keys.ENTER).build().perform();
