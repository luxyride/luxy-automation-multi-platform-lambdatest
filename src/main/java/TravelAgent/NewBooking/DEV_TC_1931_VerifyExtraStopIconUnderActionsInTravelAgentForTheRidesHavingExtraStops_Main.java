...
public class DEV_TC_1931_VerifyExtraStopIconUnderActionsInTravelAgentForTheRidesHavingExtraStops_Main extends TestBase {
	...
	@FindBy(xpath = "//button[@title='Extra Stops']")
	WebElement extraStopBtn;
	...
	public DEV_TC_1931_VerifyExtraStopIconUnderActionsInTravelAgentForTheRidesHavingExtraStops_Main(WebDriver driver) {
		try {
			this.driver = driver;
	...
	public Boolean verifyExtraStopButton(Boolean visibilityStatus) {
		try {
			if (extraStopBtn.isDisplayed())
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception e) {
