...
public class DEV_TC_1270_VerifyTheFunctionalityOfTravelArrangerProfileUnderActionsInManageTravelArrangerPage_Main
		extends TestBase {
	...
	public DEV_TC_1270_VerifyTheFunctionalityOfTravelArrangerProfileUnderActionsInManageTravelArrangerPage_Main(
			WebDriver driver) {
		try {
			this.driver = driver;
	...
	public void enterSearchField() {
		try {
			action = new Actions(driver);
			searchField.sendKeys(prop.getProperty("phoneNumber"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public Boolean visibilityOfTravelArrangerProfile(Boolean visibilityStatus) {
		try {
	...
	public void clickOnBackBtn() {
		try {
			action = new Actions(driver);
			objTestBase.defaultWaitTime(1000);
			action.moveToElement(backBtn).click().build().perform();
			objTestBase.defaultWaitTime(1000);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public Boolean visibilityOfManageTravelArrangerProfile(Boolean visibilityStatus) {
		try {
