...
public class DEV_TC_1738_VerifyTheAvailabilityOfOptInVerificationSecurityCheckboxInDispatchTravelArranger_Main extends TestBase {
	...
	public DEV_TC_1738_VerifyTheAvailabilityOfOptInVerificationSecurityCheckboxInDispatchTravelArranger_Main(WebDriver driver) {
		try {
			this.driver = driver;
	...
	public void enterSearchField() {
		try {
			action = new Actions(driver);
			searchField.sendKeys(prop.getProperty("travelAgentEMail"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public Boolean visibilityOfDescription(Boolean visibilityStatus) {
		try {
			waitTimeForElement(description);
			if (description.isDisplayed()) {
				expected = description.getText();
				if (expected.toLowerCase().contains("employ")) {
					visibilityStatus = true;
				} else {
					visibilityStatus = false;
				}
