...
public class DEV_TC_1208_VerifyTravelAgentAccountDetailsCreatedThroughWebPortalIsListedUnderManageTravelAgentInDispatch_Main
		extends TestBase {
	...
	public DEV_TC_1208_VerifyTravelAgentAccountDetailsCreatedThroughWebPortalIsListedUnderManageTravelAgentInDispatch_Main(
			WebDriver driver) {
		try {
			this.driver = driver;
	...
	public Boolean verifyTravelAgentTabNavigation(Boolean visibilityStatus) {
		try {
			action = new Actions(driver);
			objTestBase.defaultWaitTime(1000);
			...
			if (travelAgentHeader.isDisplayed())
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception ex) {
	...
	public Boolean verifyTravelAgentRegisteredDetails(Boolean visibilityStatus, String eMail) {
		try {
			action = new Actions(driver);
			objTestBase.defaultWaitTime(1000);
			...
			if (editTravelAgent.isDisplayed())
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception ex) {
