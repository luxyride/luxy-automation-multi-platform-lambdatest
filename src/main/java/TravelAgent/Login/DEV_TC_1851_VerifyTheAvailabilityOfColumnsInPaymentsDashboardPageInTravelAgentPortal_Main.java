...
public class DEV_TC_1851_VerifyTheAvailabilityOfColumnsInPaymentsDashboardPageInTravelAgentPortal_Main extends TestBase {
	...
	public DEV_TC_1851_VerifyTheAvailabilityOfColumnsInPaymentsDashboardPageInTravelAgentPortal_Main(WebDriver driver) {
		try {
			this.driver = driver;
	...
	public Boolean veriftTheColumnsAvailableInPaymentsDashBoard(Boolean visibilityStatus) {
		try {
			if (pickUp.isDisplayed()&& tripId.isDisplayed()&& dateTime.isDisplayed()&& status.isDisplayed()&& comissioncolumn.isDisplayed()&&basePrice.isDisplayed())
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception e) {
