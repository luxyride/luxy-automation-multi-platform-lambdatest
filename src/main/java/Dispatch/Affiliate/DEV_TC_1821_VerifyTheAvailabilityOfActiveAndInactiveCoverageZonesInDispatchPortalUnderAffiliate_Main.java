...
public class DEV_TC_1821_VerifyTheAvailabilityOfActiveAndInactiveCoverageZonesInDispatchPortalUnderAffiliate_Main
		extends TestBase {
	...
	List<WebElement> inactiveCoverageZonesTab;
	
	@FindBy(xpath = "//a[normalize-space()='Inactive Coverage Zones']")
	List<WebElement> activeCoverageZonesTab;

	TestBase objTestBase;

	public DEV_TC_1821_VerifyTheAvailabilityOfActiveAndInactiveCoverageZonesInDispatchPortalUnderAffiliate_Main(
			WebDriver driver) {
		try {
			this.driver = driver;
	...
	public Boolean editAffiliatePageNavigation(Boolean visibilityStatus) {
		try {
			...
			if (temp.contains("edit")) {
				...
				js.executeScript("arguments[0].scrollIntoView();", addCoverzoneBtn.get(0) , searchZone.get(0) , activeCoverageZonesTab.get(0),inactiveCoverageZonesTab.get(0) );
	...
	public Boolean searchCoverzone(Boolean visibilityStatus) {
		try {
			js = (JavascriptExecutor) driver;
			js.executeScript("arguments[0].scrollIntoView();", searchAffiliate);
			js.executeScript("window.scrollBy(0,-100)", "");
			objTestBase.defaultWaitTime(2000);
