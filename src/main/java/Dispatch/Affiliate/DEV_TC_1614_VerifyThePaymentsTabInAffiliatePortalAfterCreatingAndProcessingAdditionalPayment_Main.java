...
public class DEV_TC_1614_VerifyThePaymentsTabInAffiliatePortalAfterCreatingAndProcessingAdditionalPayment_Main
		extends TestBase {
	...
	public DEV_TC_1614_VerifyThePaymentsTabInAffiliatePortalAfterCreatingAndProcessingAdditionalPayment_Main(
			WebDriver driver) {
		try {
			this.driver = driver;
	...
	public Boolean verifyAffiliatesPaymentsFillingDetails(Boolean visibilityStatus) {
		try {
			...
			action.moveToElement(affiliateConfirmationAmount).click()
					.sendKeys(prop.getProperty("affiliateConfirmationAdditionAmount")).build().perform();
			objTestBase.defaultWaitTime(1000);
			action.moveToElement(affiliateConfirmationReason).click()
					.sendKeys(prop.getProperty("affiliateConfirmationAdditionReason")).build().perform();
			objTestBase.defaultWaitTime(1000);

			action.moveToElement(affiliatePaymentMethod).click().build().perform();
			objTestBase.defaultWaitTime(1000);
			action.moveToElement(affiliatePaymentMethod).sendKeys(prop.getProperty("affiliatePaymentMethodAddition"));
			objTestBase.defaultWaitTime(3000);
	...
	public Boolean visibilityOfPaymentswithNotProcessedstatus(Boolean visibilityStatus) {
		try {
