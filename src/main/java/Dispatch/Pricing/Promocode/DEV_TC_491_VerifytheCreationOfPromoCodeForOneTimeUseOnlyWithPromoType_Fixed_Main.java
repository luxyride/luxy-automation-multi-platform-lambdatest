...
public class DEV_TC_491_VerifytheCreationOfPromoCodeForOneTimeUseOnlyWithPromoType_Fixed_Main extends TestBase {
	...
	public DEV_TC_491_VerifytheCreationOfPromoCodeForOneTimeUseOnlyWithPromoType_Fixed_Main(WebDriver driver) {
		try {
			this.driver = driver;
	...
	public Boolean dispatchVerifyPromoTypeAndOneTimeUseOnlyStatus(Boolean visibilityStatus) {
		try {
			...
			objTestBase.defaultWaitTime(1000);
			if (!dispatchFixedchckbox.isSelected())
	...
	public void enterPromocode() {
		try {
			...
			action.moveToElement(promoCode).sendKeys(prop.getProperty("promoCodeNameFixed")).build().perform();
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
