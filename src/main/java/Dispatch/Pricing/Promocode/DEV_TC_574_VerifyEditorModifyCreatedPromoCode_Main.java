...
public class DEV_TC_574_VerifyEditorModifyCreatedPromoCode_Main extends TestBase {
	...
	@FindBy(xpath = "//*[@id=\"root\"]/div/div[3]/div/div[2]/div/div/div/div/div/div/div/div/form/div[4]/div[1]/div/div/input")
	...
	@FindBy(xpath = "//button[@type='submit']")
	WebElement dispatchPromocodeUpdateBtn;
	...
	@FindBy(xpath = "//tbody/tr[1]/td[1]/div[1]/a[1]/i[1]")
	WebElement dispatchPromocodeEditBtn;
	...
	public DEV_TC_574_VerifyEditorModifyCreatedPromoCode_Main(WebDriver driver) {
		try {
			this.driver = driver;
...
//					"2FA Validation Status for Script - " + getClass().getSimpleName() + " = " + visibilityStatus);
	...
	public Boolean dispatchvisibilityOfPromocodeTabPage1(Boolean visibilityStatus) {
		try {
			action = new Actions(driver);
			action.moveToElement(dispatchPromocodeTab).click().build().perform();
			objTestBase.defaultWaitTime(5000);
	...
	public Boolean dispatchEditionOfPromocode(Boolean visibilityStatus) {
		try {
			action = new Actions(driver);
			...
			WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10));
			WebElement displayStatus = wait.until(ExpectedConditions.visibilityOf(dispatchPromocodeEditBtn));

			if (displayStatus.isDisplayed()) {
				action.moveToElement(dispatchPromocodeEditBtn).click().build().perform();
				objTestBase.defaultWaitTime(1000);

				action = new Actions(driver);
				action.moveToElement(dispatchPromocodeName).click().sendKeys(prop.getProperty("promoCodeNameFixedEdit"))
						.build().perform();
				objTestBase.defaultWaitTime(500);

				action = new Actions(driver);
				action.moveToElement(dispatchPromocodeUpdateBtn).click().build().perform();
