...
public class DEV_TC_1051_VerifyPINConfirmationEmailIsNotSentToCustomerWhenTheRidesEdited_Main extends TestBase {
	...
	public DEV_TC_1051_VerifyPINConfirmationEmailIsNotSentToCustomerWhenTheRidesEdited_Main(WebDriver driver) {
		this.driver = driver;
...
//					"2FA Validation Status for Script - " + getClass().getSimpleName() + " = " + visibilityStatus);
	...
	public Boolean verifyEDITRideDetailsPage(Boolean visibilityStatus) {
		try {
			...
			Boolean displayStatus = wait.until(ExpectedConditions.urlContains("bookingedit"));
		...
		try {
			...
			if (expected.toLowerCase().contains("all")) {
				...
				if (bookingRideTripId.isDisplayed()
					...
					{
						objTestBase.defaultWaitTime(2000);
						rideBookingDispatchIds.put(vehicleType.getText().toLowerCase(), tripID);
						objTestBase.defaultWaitTime(2000);
					}
