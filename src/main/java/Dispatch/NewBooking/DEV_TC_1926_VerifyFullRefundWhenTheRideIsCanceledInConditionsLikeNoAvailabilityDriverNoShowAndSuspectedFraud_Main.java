...
public class DEV_TC_1926_VerifyFullRefundWhenTheRideIsCanceledInConditionsLikeNoAvailabilityDriverNoShowAndSuspectedFraud_Main
		extends TestBase {
	...
	public DEV_TC_1926_VerifyFullRefundWhenTheRideIsCanceledInConditionsLikeNoAvailabilityDriverNoShowAndSuspectedFraud_Main(
			WebDriver driver) {
		try {
			this.driver = driver;
...
//					"2FA Validation Status for Script - " + getClass().getSimpleName() + " = " + visibilityStatus);
	...
	public Boolean visibilityOfCancelButton(Boolean visibilityStatus) {
		try {
			if (confirmBookingBtn.isDisplayed())
				visibilityStatus = true;
			else
	...
	public Boolean visibilityOfReviewCancleRidepopup(Boolean visibilityStatus) {
		try {
			if (reviewCancleRidePopup.isDisplayed()) {
				...
				select.selectByValue("Suspected fraud");
				objTestBase.defaultWaitTime(2000);
