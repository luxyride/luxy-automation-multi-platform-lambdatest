...
public class DEV_TC_1908_VerifyEditRideFunctionalityWithAnyOneAvailablePaymentMethodsForDispatchPortal_Main extends TestBase {
	...
	public DEV_TC_1908_VerifyEditRideFunctionalityWithAnyOneAvailablePaymentMethodsForDispatchPortal_Main(WebDriver driver) {
		try {
			this.driver = driver;
...
//					"2FA Validation Status for Script - " + getClass().getSimpleName() + " = " + visibilityStatus);
	...
	public void selectTravelWithPet() {
		try {
			action = new Actions(driver);
			action.moveToElement(extrasPet).click().build().perform();
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
