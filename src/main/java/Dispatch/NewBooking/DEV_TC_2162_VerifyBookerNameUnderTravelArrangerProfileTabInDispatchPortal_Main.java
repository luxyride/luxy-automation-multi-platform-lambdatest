...
public class DEV_TC_2162_VerifyBookerNameUnderTravelArrangerProfileTabInDispatchPortal_Main extends TestBase {
	...
	WebElement taProfileIcon;
	
	

	TestBase objTestBase;

	public DEV_TC_2162_VerifyBookerNameUnderTravelArrangerProfileTabInDispatchPortal_Main(WebDriver driver) {
		try {
			this.driver = driver;
...
//					"2FA Validation Status for Script - " + getClass().getSimpleName() + " = " + visibilityStatus);
	...
	public Boolean visibilityOfManageTaPage(Boolean visibilityStatus) {
		try {
			waitTimeForElement(taPage);
			if (taPage.isDisplayed()) {
				objTestBase.defaultWaitTime(1000);
				if (taPage.getText().toLowerCase().contains("Passenger")) {
					visibilityStatus = true;
				} else {
					visibilityStatus = false;
				}
	...
	public void enterSearchInput() {
		try {
			action = new Actions(driver);
			searchInput.sendKeys(prop.getProperty("taEmail"));
			action.sendKeys(Keys.TAB).build().perform();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void clickOnProfileIcon() {
		try {
			action = new Actions(driver);
			objTestBase.defaultWaitTime(1000);
			action.moveToElement(taProfileIcon).click().build().perform();
			objTestBase.defaultWaitTime(1000);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
