...
public class DEV_TC_1766_VerifyTheFunctionalityOfFarmoutToAllEligiblePartnersToggleinEDITModeinDispatchPortal_Main
		extends TestBase {
	...
	@FindBy(xpath = "//span[text()='Farm Out to All Eligible Partners']")
	WebElement farmoutPartnersBtn;

	@FindBy(xpath = "(//button[text()='Yes'])[1]")
	WebElement yesBtn;

	public DEV_TC_1766_VerifyTheFunctionalityOfFarmoutToAllEligiblePartnersToggleinEDITModeinDispatchPortal_Main(
			WebDriver driver) {
		try {
			this.driver = driver;
	...
	public void editRideDetails() {
		try {
			...
			WebElement displayStatus = wait.until(ExpectedConditions.visibilityOf(editRideDetailsBtn));
	...
	public Boolean verifyFarmoutDetailsInEDITView(Boolean visibilityStatus, String claimRideHoursBeforeUpdate) {
		try {
			expected = driver.getCurrentUrl();
			...
			if (expected.contains("edit")) {
				...
				if (claimRideHours.size() != 0) {
					expected = claimRideHours.get(0).getText();
					String expectedTemp[] = expected.split(" ");
					String claimRideHoursBeforeUpdateTemp[] = claimRideHoursBeforeUpdate.split(" ");
					if (expectedTemp[0].contains(claimRideHoursBeforeUpdateTemp[0]))
						visibilityStatus = true;
					else
						visibilityStatus = false;
				}
	...
	public String updateFarmoutPreferencesClaimRideHours(String claimRideHoursAfterUpdate) {
		try {
			...
			if (claimRideHours.size() != 0) {
				...
				action = new Actions(driver);
				action.moveToElement(farmoutPartnersBtn).click().build().perform();
				defaultWaitTime(3000);
				action.moveToElement(yesBtn).click().build().perform();
				defaultWaitTime(8000);

				claimRideHoursAfterUpdate = claimRideHours.get(0).getText();
				defaultWaitTime(1000);
			} else
				claimRideHoursAfterUpdate = "";
		} catch (Exception e) {
			e.printStackTrace();
			claimRideHoursAfterUpdate = "";
		}
		return claimRideHoursAfterUpdate;
	}
}
