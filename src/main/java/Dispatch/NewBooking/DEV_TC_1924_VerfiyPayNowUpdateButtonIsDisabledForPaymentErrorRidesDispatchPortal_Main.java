...
public class DEV_TC_1924_VerfiyPayNowUpdateButtonIsDisabledForPaymentErrorRidesDispatchPortal_Main extends TestBase {
	...
	@FindBy(xpath = "//span[normalize-space()='Update Ride']")
	WebElement updateRideBtn;
	...
	public DEV_TC_1924_VerfiyPayNowUpdateButtonIsDisabledForPaymentErrorRidesDispatchPortal_Main(WebDriver driver) {
		try {
			this.driver = driver;
	...
	public void addToAddress() {
		try {
			action = new Actions(driver);
			toAddress.sendKeys(prop.getProperty("toAddressAirport"));
			objTestBase.defaultWaitTime(3000);
			action.sendKeys(Keys.DOWN).build().perform();
			objTestBase.defaultWaitTime(1000);
			action.sendKeys(Keys.ENTER).build().perform();
	...
	public Boolean verifyRideBookingConfirmation(Boolean visibilityStatus) {
		try {
			...
			if (displayStatus.isDisplayed()) {
				...
				visibilityStatus = false;
			objTestBase.defaultWaitTime(1000);
			if (visibilityStatus == true && msgText.contains("Process"))
				visibilityStatus = true;
			else
				visibilityStatus = false;
	...
	public Boolean visibilityOfUpdateRideButtonDisabled(Boolean visibilityStatus) {
		try {
			if (!updateRideBtn.isDisplayed())
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception e) {
	...
	public Boolean visibilityOfPayNowButtonDisabled(Boolean visibilityStatus) {
		try {
