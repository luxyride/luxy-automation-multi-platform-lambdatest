...
public class DEV_TC_2072_VerifyTheFunctionalityOfTripSourceDropdownInDispatchPortal_Main extends TestBase {
	...
	public DEV_TC_2072_VerifyTheFunctionalityOfTripSourceDropdownInDispatchPortal_Main(WebDriver driver) {
		try {
			this.driver = driver;
...
//					"2FA Validation Status for Script - " + getClass().getSimpleName() + " = " + visibilityStatus);
	...
	public Boolean visibilityOfConciergeRidesList(Boolean visibilityStatus) {
		try {
			if (conciergeRidesList.isDisplayed())
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception e) {
	...
	public void clickOnTravelArranger() {
		try {
			action = new Actions(driver);
			objTestBase.defaultWaitTime(1000);
	...
	public Boolean visibilityOfTravelArrangerRidesList(Boolean visibilityStatus) {
		try {
			if (travelArrangerRidesList.isDisplayed())
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception e) {
	...
	public Boolean visibilityOfPhoneList(Boolean visibilityStatus) {
		try {
			if (phoneRidesList.isDisplayed())
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception e) {
	...
	public Boolean visibilityOfWebList(Boolean visibilityStatus) {
		try {
			if (webRidesList.isDisplayed())
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception e) {
	...
	public Boolean visibilityOfMobileList(Boolean visibilityStatus) {
		try {
			if (mobileRidesList.isDisplayed())
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception e) {
	...
	public void clickOnstarred() {
		try {
			action = new Actions(driver);
			objTestBase.defaultWaitTime(1000);
			action.moveToElement(starred).click().build().perform();
			objTestBase.defaultWaitTime(1000);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public Boolean visibilityOfStarredList(Boolean visibilityStatus) {
		try {
			if (starredRidesList.isDisplayed())
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception e) {
