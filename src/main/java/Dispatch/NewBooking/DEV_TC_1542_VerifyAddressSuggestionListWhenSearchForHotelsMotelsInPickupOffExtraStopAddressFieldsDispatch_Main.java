...
public class DEV_TC_1542_VerifyAddressSuggestionListWhenSearchForHotelsMotelsInPickupOffExtraStopAddressFieldsDispatch_Main
		extends TestBase {
	...
	public DEV_TC_1542_VerifyAddressSuggestionListWhenSearchForHotelsMotelsInPickupOffExtraStopAddressFieldsDispatch_Main(
			WebDriver driver) {
		try {
			this.driver = driver;
	...
	public Boolean addFromAddress(Boolean visibilityStatus) {
		try {
			...
			objTestBase.defaultWaitTime(1000);
			fromAddress.sendKeys(prop.getProperty("motelAddress"));
	...
	public Boolean addToAddress(Boolean visibilityStatus) {
		try {
			action = new Actions(driver);
			toAddress.sendKeys(prop.getProperty("groundAddress"));
			objTestBase.defaultWaitTime(6000);
			action.sendKeys(Keys.DOWN).build().perform();
			objTestBase.defaultWaitTime(1000);
			action.sendKeys(Keys.ENTER).build().perform();
	...
	public Boolean addExtraStop(Boolean visibilityStatus) {
		try {
			action = new Actions(driver);
			addextraStop.sendKeys(prop.getProperty("hotelAddress"));
			objTestBase.defaultWaitTime(6000);
			action.sendKeys(Keys.DOWN).build().perform();
			objTestBase.defaultWaitTime(1000);
			action.sendKeys(Keys.ENTER).build().perform();
