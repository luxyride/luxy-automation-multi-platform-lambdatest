...
public class DEV_TC_2172_VerifyThatUserCanDownloadTheCompleteRideHistoryForAPassengerPastTrips_Main extends TestBase {
	...
	@FindBy(xpath = "//button[normalize-space()='Ride History']")
	WebElement rideHistory;


	TestBase objTestBase;

	public DEV_TC_2172_VerifyThatUserCanDownloadTheCompleteRideHistoryForAPassengerPastTrips_Main(WebDriver driver) {
		try {
			this.driver = driver;
	...
	public Boolean visibilityOfRideHistory(Boolean visibilityStatus) {
		try {
			if (rideHistory.isDisplayed())
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception e) {
