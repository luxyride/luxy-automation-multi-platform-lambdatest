...
public class DEV_TC_1879_VerifyTheSortingFunctionalityOfBucketNameForAffiliatesInDispatchPortal_Main
		extends TestBase {
	...
	@FindBy(xpath = "//th[@title='Bucket Name']")
	WebElement bucketName;
	...
	public DEV_TC_1879_VerifyTheSortingFunctionalityOfBucketNameForAffiliatesInDispatchPortal_Main(
			WebDriver driver) {
		try {
			this.driver = driver;
	...
	public Boolean verifyBucketNameInManageAffiliatePage(Boolean visibilityStatus) {
		try {
			if (bucketName.isDisplayed())
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception e) {
