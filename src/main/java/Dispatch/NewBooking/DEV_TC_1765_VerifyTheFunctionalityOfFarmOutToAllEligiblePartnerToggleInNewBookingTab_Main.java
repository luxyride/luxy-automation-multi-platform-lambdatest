...
public class DEV_TC_1765_VerifyTheFunctionalityOfFarmOutToAllEligiblePartnerToggleInNewBookingTab_Main extends TestBase {
	...
	@FindBy(xpath = "(//label[normalize-space()='Do you allow pet service?'])[1]")
	WebElement petService;
	
	@FindBy(xpath = "(//label[normalize-space()='Do you provide toddler seat 1-4 years?'])[1]")
	WebElement childSeats;

	@FindBy(xpath = "//input[@name='isFarmoutToAll']")
	...
	WebElement bookingRidetripIDFarmoutScenario;
	...
	@FindBy(xpath = "(//h1[normalize-space()='Alert'])[1]")
	...
	@FindBy(xpath = "(//span[text()='Affiliate Should Claim a ride within'])[1]")
	WebElement claimRideHoursTextMsg;

	@FindBy(xpath = "(//span[text()='Affiliate Should Claim a ride within'])[1]//following::select[@name='age']")
	List<WebElement> claimRideHours;
	...
	public DEV_TC_1765_VerifyTheFunctionalityOfFarmOutToAllEligiblePartnerToggleInNewBookingTab_Main(WebDriver driver) {
		try {
			this.driver = driver;
...
//					"2FA Validation Status for Script - " + getClass().getSimpleName() + " = " + visibilityStatus);
	...
	public Boolean visibilityOfFarmOutToggleBtn(Boolean visibilityStatus) {
		try {
	...
	public Boolean visibilityOfExtras(Boolean visibilityStatus) {
		try {
			if (petService.isDisplayed() && childSeats.isDisplayed())
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception e) {
	...
	public void searchRideId() {
		try {
			searchRideId.sendKeys(prop.getProperty("dispatchadminMail"));
			objTestBase.defaultWaitTime(1000);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public String captureRideBookingIDs(Boolean visibilityStatus, String scenario, String tripIDFarmoutScenarioFarmoutScenario) {
		try {
			action = new Actions(driver);
			ridesTabAll.click();
			...
			if (expected.toLowerCase().contains("all")) {
				...
				if (bookingRidetripIDFarmoutScenario.isDisplayed()
						&& !rideBookingDispatchIds.containsValue(bookingRidetripIDFarmoutScenario.getText())) {
							...
							&& ridePlaceDateTime.getText().toLowerCase().contains(currDate))// 06-27-2023 12:00 AM
					{
						...
						rideBookingDispatchIds.put(vehicleType.getText().toLowerCase(),
								bookingRidetripIDFarmoutScenario.getText().toString());
						objTestBase.defaultWaitTime(500);
						tripIDFarmoutScenario = bookingRidetripIDFarmoutScenario.getText().toString();
						objTestBase.defaultWaitTime(1000);
						testStatus = "PASSED";
					} else
						testStatus = "FAILED";
	...
	public void enableFarmoutToggle() {
		try {
			toggleBtn.click();
			objTestBase.defaultWaitTime(2000);
		} catch (Exception e) {
	...
	public Boolean visibilityOfConfirmPopUp(Boolean visibilityStatus) {
		try {
	...
	public String captureFarmoutPreferencesClaimRideHoursBeforeUpdate(String claimRideHoursBeforeUpdate) {
		try {
			...
			defaultWaitTime(500);
			if (claimRideHours.size() != 0) {
				Select select = new Select(claimRideHours.get(0));
				WebElement option = select.getFirstSelectedOption();
				claimRideHoursBeforeUpdate = option.getText();
			} else
		...
		System.out.println(claimRideHoursBeforeUpdate);
		return claimRideHoursBeforeUpdate;
	}
}
