...
public class DEV_TC_1780_HourlyVerifyBookingARideinHourlyTabDispatchByDisablingSameAsPickupAddressCheckBox_Main extends TestBase {
	...
	public DEV_TC_1780_HourlyVerifyBookingARideinHourlyTabDispatchByDisablingSameAsPickupAddressCheckBox_Main(WebDriver driver) {
		try {
			this.driver = driver;
...
//					"2FA Validation Status for Script - " + getClass().getSimpleName() + " = " + visibilityStatus);
	...
	public void unUncheckSameAsPickUp() {
		try {
			action = new Actions(driver);
			objTestBase.defaultWaitTime(1000);
			action.moveToElement(checkBox).click().build().perform();
	...
	public void clickOncheckbox() {
		try {
			action = new Actions(driver);
			action.moveToElement(searchCheckBox).click().build().perform();
			objTestBase.defaultWaitTime(1000);
		} catch (Exception e) {
			e.printStackTrace();
		}
