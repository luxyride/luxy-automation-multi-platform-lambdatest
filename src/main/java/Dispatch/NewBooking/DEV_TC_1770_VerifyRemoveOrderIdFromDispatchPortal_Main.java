...
public class DEV_TC_1770_VerifyRemoveOrderIdFromDispatchPortal_Main extends TestBase {
	...
	public DEV_TC_1770_VerifyRemoveOrderIdFromDispatchPortal_Main(WebDriver driver) {
		try {
			this.driver = driver;
...
//					"2FA Validation Status for Script - " + getClass().getSimpleName() + " = " + visibilityStatus);
		...
		try {
			...
			if (expected.toLowerCase().contains("all")) {
				...
				if (bookingRideTripId.isDisplayed()
					 ...
					 * ridePlaceDateTime.getText().toLowerCase().contains(currDate))// 06-27-2023
					 * 12:00 AM {
					 */
					objTestBase.defaultWaitTime(1000);
	...
	public void enterSearchInput() {
		try {
			companyName.sendKeys(prop.getProperty("affiliateCompanyName"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
