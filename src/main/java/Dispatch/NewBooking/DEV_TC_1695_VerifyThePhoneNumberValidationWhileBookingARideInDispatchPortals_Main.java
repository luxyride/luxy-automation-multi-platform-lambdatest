...
public class DEV_TC_1695_VerifyThePhoneNumberValidationWhileBookingARideInDispatchPortals_Main extends TestBase {
	...
	@FindBy(xpath = "//span[normalize-space()='Invalid Mobile Number']")
	WebElement alertMessgeInvalidMobileNor;
	...
	@FindBy(xpath = "(//b[contains(text(),'Mobile # :')])[1]")
	...
	public DEV_TC_1695_VerifyThePhoneNumberValidationWhileBookingARideInDispatchPortals_Main(WebDriver driver) {
		try {
			this.driver = driver;
...
//					"2FA Validation Status for Script - " + getClass().getSimpleName() + " = " + visibilityStatus);
	...
	public void enterspMobileInvalid() {
		try {
			spMobile.sendKeys(prop.getProperty("spMobileInvalid"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}

	public void enterMobileInvalid() {
		try {
			defaultWaitTime(1000);
			Mobile.sendKeys(Keys.CONTROL + "A");
			Mobile.sendKeys(Keys.DELETE);
			defaultWaitTime(2000);
			Mobile.sendKeys(prop.getProperty("spMobileInvalid"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public Boolean verifyRideBookingConfirmation(Boolean visibilityStatus) {
		try {
			...
			objTestBase.defaultWaitTime(1000);
			if (displayStatus.isDisplayed() && msgText.contains("successfully"))
				visibilityStatus = true;
			else
				visibilityStatus = false;
	...
	public Boolean verifyInternalNotesExpandedView(Boolean visibilityStatus) {
		try {
			action = new Actions(driver);
	...
	public Boolean verifyAlertMessage(Boolean visibilityStatus) {
		try {
	...
	public Boolean verifyAlertMessageUnderPhoneNumber(Boolean visibilityStatus) {
		try {
			String msgText = "";
			WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(20));
			WebElement displayStatus = wait.until(ExpectedConditions.visibilityOf(alertMessgeInvalidMobileNor));
			objTestBase.defaultWaitTime(500);
			msgText = alertMessgeInvalidMobileNor.getText().toLowerCase();
			objTestBase.defaultWaitTime(1000);
			if (displayStatus.isDisplayed() && msgText.contains("invalid mobile number"))
				visibilityStatus = true;
			else
				visibilityStatus = false;
			System.out.println(msgText);
	...
	public Boolean visibilityOfMobileField(Boolean visibilityStatus) {
		try {
	...
	public void enterspMobile() {
		try {
			defaultWaitTime(1000);
			spMobile.sendKeys(Keys.CONTROL + "A");
			spMobile.sendKeys(Keys.DELETE);
			spMobile.sendKeys(prop.getProperty("phoneNumber"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
