...
public class DEV_TC_1950_VerifyTheAvailabilityLevelOfInsuranceOptionsForPartnersInAffiliatePortal_Main
		extends TestBase {
	WebDriver driver;
	Actions action;
	String expected;
	...
	public DEV_TC_1950_VerifyTheAvailabilityLevelOfInsuranceOptionsForPartnersInAffiliatePortal_Main(
			WebDriver driver) {
		this.driver = driver;
	...
	public Boolean visibilityOfCommercialDropDown(Boolean visibilityStatus) {
		try {
			if (commercialSearch.isDisplayed())
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception ex) {
	...
	public void clickOnSelectDropDown() {
		try {
			action = new Actions(driver);
			objTestBase.defaultWaitTime(1000);
			action.moveToElement(commercialSearch).click().build().perform();
			objTestBase.defaultWaitTime(1000);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public Boolean visibilityOfCommercialDropDownList(Boolean visibilityStatus) {
		try {
	...
	public void clickOnCompanyProfile() {
		try {
			action = new Actions(driver);
			objTestBase.defaultWaitTime(1000);
			action.moveToElement(companyProfileTab).click().build().perform();
			objTestBase.defaultWaitTime(1000);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
