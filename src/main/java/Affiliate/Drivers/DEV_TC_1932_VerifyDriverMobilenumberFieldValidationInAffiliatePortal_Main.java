...
public class DEV_TC_1932_VerifyDriverMobilenumberFieldValidationInAffiliatePortal_Main extends TestBase {
	...
	WebElement driverExpirationDate;
	...
	public DEV_TC_1932_VerifyDriverMobilenumberFieldValidationInAffiliatePortal_Main(WebDriver driver) {
		this.driver = driver;
	...
	public String enterAllDetails(String eMail) throws Exception {
		try {
			...
			driverExpirationDate.sendKeys(eMail);
	...
	public void enterInvalidMobileInput() {
		try {
			action = new Actions(driver);
			phoneInput.sendKeys(prop.getProperty("invalidPhoneNumber"));
			action.sendKeys(Keys.TAB).build().perform();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public Boolean verifyDriverAccountErrorMsg(Boolean visibilityStatus) {
		try {
			waitTimeForElement(registerBtn);
			registerBtn.click();
			...
			if (displayStatus.isDisplayed() && alertMessge.getText().toLowerCase().contains("Please check all required fields and validation"))
				visibilityStatus = true;
			else
				visibilityStatus = false;
	...
	public void enterValidMobileInput() {
		try {
			action = new Actions(driver);
			phoneInput.clear();
