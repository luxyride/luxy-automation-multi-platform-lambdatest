...
public class DEV_TC_1868_VerifyTheRideBookingWhenZoneTypeAsStateAndSetTheConfigurationBufferTimeAndMiles_Main
		extends TestBase {
...
//	WebElement passengerNotes;
	...
	public DEV_TC_1868_VerifyTheRideBookingWhenZoneTypeAsStateAndSetTheConfigurationBufferTimeAndMiles_Main(
			WebDriver driver) {
		try {
			this.driver = driver;
	...
	public Map<String, Integer> enterTime(String scenario, Boolean lessThan24hrs, Map<String, Integer> timeHHMM) {
		try {
			...
			utillLogger.info(
					"DEV_TC_1868_VerifyTheRideBookingWhenZoneTypeAsStateAndSetTheConfigurationBufferTimeAndMiles_Test - Actual Time in Hours = "
	...
	public Boolean verifyVisibilityOfWarningMsg(Boolean visibilityStatus) {
		try {
			...
			if (expected != null) {
				System.out.println("Configured Hours = " + prop.getProperty("zoneTypeStateConfigTime") + "\n"
						+ "Configured Miles = " + prop.getProperty("zoneTypeStateConfigMiles"));
				if (expected.toLowerCase().contains(prop.getProperty("zoneTypeStateConfigTime"))
						&& expected.toLowerCase().contains(prop.getProperty("zoneTypeStateConfigMiles")))
					visibilityStatus = true;
				else
					visibilityStatus = false;
			} else {
	...
	public void addDispatchFromAddress() {
		try {
			...
			objTestBase.defaultWaitTime(1000);
			dispatchFromAddress.sendKeys(prop.getProperty("zoneTypeStateFromAddress"));
			objTestBase.defaultWaitTime(3000);
			action.sendKeys(Keys.DOWN).build().perform();
			objTestBase.defaultWaitTime(1000);
			action.sendKeys(Keys.ENTER).build().perform();
	...
	public void addDispatchToAddress() {
		try {
			action = new Actions(driver);
			dispatchToAddress.sendKeys(prop.getProperty("zoneTypeStateToAddressPositive"));
			objTestBase.defaultWaitTime(3000);
			action.sendKeys(Keys.DOWN).build().perform();
			objTestBase.defaultWaitTime(1000);
			action.sendKeys(Keys.ENTER).build().perform();
