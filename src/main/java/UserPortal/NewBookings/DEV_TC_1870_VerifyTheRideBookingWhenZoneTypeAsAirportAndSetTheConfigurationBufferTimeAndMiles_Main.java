...
public class DEV_TC_1870_VerifyTheRideBookingWhenZoneTypeAsAirportAndSetTheConfigurationBufferTimeAndMiles_Main
		extends TestBase {
...
//	WebElement passengerNotes;
	...
	public DEV_TC_1870_VerifyTheRideBookingWhenZoneTypeAsAirportAndSetTheConfigurationBufferTimeAndMiles_Main(
			WebDriver driver) {
		try {
			this.driver = driver;
	...
	public Map<String, Integer> enterTime(String scenario, Boolean lessThan24hrs, Map<String, Integer> timeHHMM) {
		try {
			...
			if (scenario != "positivescenario") {
				if (scenario == "lessthanBufferTime" && actualTimeHours <= 21) {
					...
					configuredTimeHours = (24 - actualTimeHours)
							- Integer.parseInt(prop.getProperty("zoneTypeAirportConfigTime") + 1);
				...
				configuredTimeHours = 24
						- (actualTimeHours - Integer.parseInt(prop.getProperty("zoneTypeAirportConfigTime")) - 1);
				int result = Integer.signum(configuredTimeHours);
				if (result == -1) {
					System.out.println(Math.abs(configuredTimeHours));
					configuredTimeHours = Math.abs(configuredTimeHours);
			...
			utillLogger.info(
					"DEV_TC_1870_VerifyTheRideBookingWhenZoneTypeAsAirportAndSetTheConfigurationBufferTimeAndMiles_Test - Actual Time in Hours = "
	...
	public Boolean verifyVisibilityOfWarningMsg(Boolean visibilityStatus) {
		try {
			...
			if (expected != null) {
				System.out.println("Configured Hours = " + prop.getProperty("zoneTypeAirportConfigTime") + "\n"
						+ "Configured Miles = " + prop.getProperty("zoneTypeAirportConfigMiles"));
				if (expected.toLowerCase().contains(prop.getProperty("zoneTypeAirportConfigTime"))
						&& expected.toLowerCase().contains(prop.getProperty("zoneTypeAirportConfigMiles")))
					visibilityStatus = true;
				else
					visibilityStatus = false;
			} else {
	...
	public void addDispatchFromAddress() {
		try {
			...
			objTestBase.defaultWaitTime(1000);
			dispatchFromAddress.sendKeys(prop.getProperty("zoneTypeAirportFromAddress"));
			objTestBase.defaultWaitTime(3000);
			action.sendKeys(Keys.DOWN).build().perform();
			objTestBase.defaultWaitTime(1000);
			action.sendKeys(Keys.ENTER).build().perform();
	...
	public void addDispatchToAddress() {
		try {
			action = new Actions(driver);
			dispatchToAddress.sendKeys(prop.getProperty("zoneTypeAirportToAddressPositive"));
			objTestBase.defaultWaitTime(3000);
			action.sendKeys(Keys.DOWN).build().perform();
			objTestBase.defaultWaitTime(1000);
			action.sendKeys(Keys.ENTER).build().perform();
