...
public class DEV_TC_2221_VerifyTheDisplayingOfErrorMessagesAndErrorHighlightingInRideBookingFlowInWebUserPortal_Main extends TestBase {
	...
	@FindBy(xpath = "//p[normalize-space()='Please select the pickup address from the list']")
	WebElement errorMsg;
	
	@FindBy(xpath = "//p[normalize-space()='Please enter the first name']")
	WebElement firstName;

	@FindBy(xpath = "//p[normalize-space()='Please enter the last name']")
	WebElement lastName;

	@FindBy(xpath = "//p[normalize-space()='Please enter the phone number']")
	WebElement mobileInput;

	@FindBy(xpath = "//p[normalize-space()='Please enter the email address']")
	WebElement emailInput;
	...
	public DEV_TC_2221_VerifyTheDisplayingOfErrorMessagesAndErrorHighlightingInRideBookingFlowInWebUserPortal_Main(WebDriver driver) {
		try {
			this.driver = driver;
	...
	public Boolean visibilityOfErrorMessages(Boolean visibilityStatus) {
		try {
			js = (JavascriptExecutor) driver;
	...
	public Boolean verifyErrorMsgsUnderPrimaryFields(Boolean visibilityStatus) {
		try {
			if (firstName.isDisplayed() && lastName.isDisplayed() && mobileInput.isDisplayed() && emailInput.isDisplayed())
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception e) {
	...
	public void clickOnArrivalMeetAndGreet() {
		try {
			action = new Actions(driver);
			js = (JavascriptExecutor) driver;
			js.executeScript("arguments[0].scrollIntoView(true);", arrivalMgBtn);
			js.executeScript("window.scrollBy(0,-100)", "");
			arrivalMgBtn.click();
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	public Boolean verifyErrorMsgsUnderPassengerName(Boolean visibilityStatus) {
		try {
