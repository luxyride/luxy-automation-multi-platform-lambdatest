...
public class DEV_TC_2369_VerifyTheAvailabilityAndFunctionalityOfBusinessReferenceFieldInWebUserPortal_Main
		extends TestBase {
	...
	@FindBy(xpath = "//*[normalize-space()='Business Travel']")
	WebElement bookingForBussiness;

	@FindBy(xpath = "(//input[@id='p_booking_reference'])")
	WebElement inputBussiness;
	...
	public DEV_TC_2369_VerifyTheAvailabilityAndFunctionalityOfBusinessReferenceFieldInWebUserPortal_Main(
			WebDriver driver) {
		try {
			this.driver = driver;
	...
	public Boolean verifyBookingForBussinessSelected(Boolean visibilityStatus) {
		try {
			bookingForBussiness.click();
			if (bookingForBussiness.isDisplayed())
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception e) {
	...
	public void enterBussinessInput() {
		try {
			defaultWaitTime(1000);
			inputBussiness.sendKeys(prop.getProperty("pasengerNotes"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
