...
public class DEV_TC_2175_VerifyChangeVerbiageInPromoCodeFieldInWebUserPortal_Main extends TestBase {
	...
	public DEV_TC_2175_VerifyChangeVerbiageInPromoCodeFieldInWebUserPortal_Main(WebDriver driver) {
		try {
			this.driver = driver;
	...
	public void enterPromocode() {
		try {
			...
			js.executeScript("window.scrollBy(0,-100)", "");
			promoCode.sendKeys(prop.getProperty("primaryEmail"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public Boolean verifyVerbiageChangeInPromoCodeField(Boolean visibilityStatus) {
		try {
			JavascriptExecutor js = (JavascriptExecutor) driver;
			js.executeScript("arguments[0].scrollIntoView(true);", promoCode);
			js.executeScript("window.scrollBy(0,-100)", "");
			if (promoCode.isDisplayed()) {
				expected = promoCode.getText();
			} else {
				visibilityStatus = false;
			}
		} catch (Exception e) {
