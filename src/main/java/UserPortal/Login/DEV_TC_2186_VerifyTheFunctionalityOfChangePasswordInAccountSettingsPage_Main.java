...
public class DEV_TC_2186_VerifyTheFunctionalityOfChangePasswordInAccountSettingsPage_Main extends TestBase {
	...
	@FindBy(xpath = "//input[@id='existing_password']")
	...
	public DEV_TC_2186_VerifyTheFunctionalityOfChangePasswordInAccountSettingsPage_Main(WebDriver driver) {
		try {
			this.driver = driver;
	...
	public Boolean VerifyAccountsSettingsPage(Boolean visibilityStatus) {
		try {
			js = (JavascriptExecutor) driver;
			js.executeScript("arguments[0].scrollIntoView(true);", oldPassword);
			js.executeScript("window.scrollBy(0,-100)", "");
			expected = driver.getCurrentUrl();
			if (oldPassword.isDisplayed() && expected.contains("settings"))
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception e) {
	...
	public void enterOldPassword() {
		try {
			...
			js.executeScript("arguments[0].scrollIntoView(true);", oldPassword);
			js.executeScript("window.scrollBy(0,-100)", "");
			oldPassword.sendKeys(prop.getProperty("newPassword"));
			action.sendKeys(Keys.TAB).build().perform();
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public void enternewPassword() {
		try {
			action = new Actions(driver);
			js = (JavascriptExecutor) driver;
			js.executeScript("arguments[0].scrollIntoView(true);", newPassword);
			js.executeScript("window.scrollBy(0,-100)", "");
			newPassword.sendKeys(prop.getProperty("newPassword"));
			action.sendKeys(Keys.TAB).build().perform();
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public Boolean VerifyErrorMessage(Boolean visibilityStatus) {
		try {
			js = (JavascriptExecutor) driver;
