...
public class DEV_TC_2189_VerifyTheAvailabilityOfFavoriteAddressesUnderMyAddressesMenu_Main extends TestBase {
	...
	@FindBy(xpath = "(//button[@value='Edit'])[1]")
	WebElement editBtn;
	...
	public DEV_TC_2189_VerifyTheAvailabilityOfFavoriteAddressesUnderMyAddressesMenu_Main(WebDriver driver) {
		try {
			this.driver = driver;
	...
	public void clickOnMyAddresses(String string) {
		try {
			action = new Actions(driver);
			js = (JavascriptExecutor) driver;
			js.executeScript("arguments[0].scrollIntoView(true);", myAddresses);
			js.executeScript("window.scrollBy(0,-100)", "");
			action.moveToElement(myAddresses).click().build().perform();
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	}

	public Boolean VerifyMyaddressesPage(Boolean visibilityStatus) {
		try {
			js = (JavascriptExecutor) driver;
			js.executeScript("arguments[0].scrollIntoView(true);", editBtn);
			js.executeScript("window.scrollBy(0,-100)", "");
			expected = driver.getCurrentUrl();
			if (editBtn.isDisplayed() && deleteBtn.isDisplayed()&& expected.contains("addresses"))
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception e) {
