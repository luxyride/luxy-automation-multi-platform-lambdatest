...
public class DEV_TC_2068_VerifyTheFunctionalityOfAddAnotherTripButtonInEventAndSpecialServiceQuoteRequestPage_Main
		extends TestBase {
	WebDriver driver;
	Actions action;
	String expected;
	...
	@FindBy(xpath = "//button[normalize-space()='Remove Trip 2']")
	WebElement removeBtn;

	TestBase objTestBase;

	public DEV_TC_2068_VerifyTheFunctionalityOfAddAnotherTripButtonInEventAndSpecialServiceQuoteRequestPage_Main(
			WebDriver driver) {
		try {
			this.driver = driver;
	...
	public void clickOnAddAnotherTrip() {
		try {
			action = new Actions(driver);
			objTestBase.defaultWaitTime(1000);
			action.moveToElement(anotherTrip).click().build().perform();
			objTestBase.defaultWaitTime(3000);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public Boolean visibilityOfRemoveTrip(Boolean visibilityStatus) {
		try {
