...
public class DEV_TC_1542_VerifyAddressSuggestionListWhenSearchForHotelsMotelsInPickupOffExtraStopAddressFieldsConcierge_Main
		extends TestBase {
	...
	public DEV_TC_1542_VerifyAddressSuggestionListWhenSearchForHotelsMotelsInPickupOffExtraStopAddressFieldsConcierge_Main(
			WebDriver driver) {
		try {
			this.driver = driver;
	...
	public Boolean addFromAddress(Boolean visibilityStatus) {
		try {
			...
			objTestBase.defaultWaitTime(1000);
			fromAddress.sendKeys(prop.getProperty("fromAddressGround"));
	...
	public Boolean addToAddress(Boolean visibilityStatus) {
		try {
			action = new Actions(driver);
			toAddress.sendKeys(prop.getProperty("toAddressMotel"));
			objTestBase.defaultWaitTime(6000);
			action.sendKeys(Keys.DOWN).build().perform();
			objTestBase.defaultWaitTime(1000);
			action.sendKeys(Keys.ENTER).build().perform();
	...
	public Boolean addExtraStop(Boolean visibilityStatus) {
		try {
			action = new Actions(driver);
			addextraStop.sendKeys(prop.getProperty("extraStopHotel"));
			objTestBase.defaultWaitTime(6000);
			action.sendKeys(Keys.DOWN).build().perform();
			objTestBase.defaultWaitTime(1000);
			action.sendKeys(Keys.ENTER).build().perform();
