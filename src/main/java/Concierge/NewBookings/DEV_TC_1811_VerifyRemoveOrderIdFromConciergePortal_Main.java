...
public class DEV_TC_1811_VerifyRemoveOrderIdFromConciergePortal_Main extends TestBase {
	...
	public DEV_TC_1811_VerifyRemoveOrderIdFromConciergePortal_Main(WebDriver driver) {
		try {
			this.driver = driver;
	...
	public void enterSearchInput() {
		try {
			companyName.sendKeys(prop.getProperty("conciergeCompanyName"));
			action.sendKeys(Keys.ENTER).build().perform();
		} catch (Exception e) {
			e.printStackTrace();
		}
