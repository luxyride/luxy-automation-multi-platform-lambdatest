...
public class DEV_TC_1522_VerifyFilteringOfConciergeRidesInConciergePortalUsingOriginatorTypeAfterRidesEditFromDispatch_Main
		extends TestBase {
	...
	public DEV_TC_1522_VerifyFilteringOfConciergeRidesInConciergePortalUsingOriginatorTypeAfterRidesEditFromDispatch_Main(
			WebDriver driver) {
		try {
			this.driver = driver;
...
//					"2FA Validation Status for Script - " + getClass().getSimpleName() + " = " + visibilityStatus1);
	...
	public Boolean visibilityOfRespectivePage(Boolean visibilityStatus, String scenario) {
		try {
			...
			if (scenario.contains("concierge_Rides_All") && expected.toLowerCase().contains("all"))
				visibilityStatus = true;
			else if (scenario.contains("concierge_InProgress") && expected.toLowerCase().contains("inprogress"))
				visibilityStatus = true;
			else if (scenario.contains("concierge_Upcoming") && expected.toLowerCase().contains("upcoming"))
				visibilityStatus = true;
			else if (scenario.contains("concierge_Available") && expected.toLowerCase().contains("available"))
				visibilityStatus = true;
	...
	public Boolean verifyRideBookingConfirmation(Boolean visibilityStatus) {
		try {
			...
			if (displayStatus.isDisplayed() && alertMessge.getText().toString().toLowerCase().contains("successfully"))
				visibilityStatus = true;
			else if (displayStatus.isDisplayed() && alertMessge.getText().toString().toLowerCase().contains("please")) {
				...
				if (displayStatus.isDisplayed()
						&& alertMessge.getText().toString().toLowerCase().contains("successfully"))
					visibilityStatus = true;
				else
					visibilityStatus = false;
			} else
	...
	public String validateTripIDDetails(String scenario, String ridePickupDate) {
		...
		if (visibilityStatus.booleanValue() == true) {
			if (!scenario.equalsIgnoreCase("concierge_Available")
				...
				visibilityStatus = true;
			} else if (scenario.equalsIgnoreCase("concierge_Available")
					&& (!rideStatus.getText().toLowerCase().contains("cancelled")
							|| !rideStatus.getText().toLowerCase().contains("completed"))
	...
	public void Originator() {
		try {
			action = new Actions(driver);
			OriginatorDropDown.click();
			objTestBase.defaultWaitTime(1000);
			Select select = new Select(OriginatorDropDown);
			select.selectByValue(prop.getProperty("OriginatorDropdown"));
			objTestBase.defaultWaitTime(2000);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public Boolean verifySourceSystemConcierge(Boolean visibilityStatus) {
		try {
			if (sourceValueConcierge.size() != 0) {
				...
				if (expected.equalsIgnoreCase("concierge"))
					visibilityStatus = true;
				else
					visibilityStatus = false;
			}
