...
public class DEV_TC_1732_VerifyFunctionalityOfClientMatterNumberInConciergePortalWhenIsClientMatterRequiredInDBIsTrue_Main
		extends TestBase {
	...
	@FindBy(xpath = "//span[contains(normalize-space(),'Client Matter Number is required')]")
	...
	public DEV_TC_1732_VerifyFunctionalityOfClientMatterNumberInConciergePortalWhenIsClientMatterRequiredInDBIsTrue_Main(
			WebDriver driver) {
		try {
			this.driver = driver;
	...
	public Boolean visibilityOfLoggedinUser(Boolean visibilityStatus) {
		try {
			...
			WebElement displayStatus = wait.until(ExpectedConditions.visibilityOf(signInBtnDropdown));
	...
	public void clickSigninButtonDispatch() {
		try {
			objTestBase.defaultWaitTime(2000);
			WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(40));
			WebElement displayStatus = wait.until(ExpectedConditions.visibilityOf(signInBtnDispatch));
			if (displayStatus.isDisplayed()) {
				action = new Actions(driver);
	...
	public Boolean verifyClientMatterNumberAfterUpdatedSuccessfully(Boolean visibilityStatus, String tripID,
			String clientMatterNumberBeforeUpdate, String clientMatterNumberAfterUpdate) {
		try {
			driver.navigate().refresh();
			objTestBase.defaultWaitTime(3000);
			searchRideIdDispatch.sendKeys(tripID);
			...
			objTestBase.defaultWaitTime(2000);
			String tempVal = clientMatterNumber.getAttribute("href");
			objTestBase.defaultWaitTime(5000);
			System.out.println("Client Matter Number After Update and then checking in EDIT Page + " + tempVal);
			utillLogger.info("Client Matter Number After Update and then checking in EDIT Page + " + tempVal);
			if (!tempVal.equalsIgnoreCase(clientMatterNumberBeforeUpdate))
				visibilityStatus = true;
			else
				visibilityStatus = false;
