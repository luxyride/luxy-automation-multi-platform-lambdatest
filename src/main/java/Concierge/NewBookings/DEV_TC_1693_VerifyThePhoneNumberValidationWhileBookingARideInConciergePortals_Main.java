...
public class DEV_TC_1693_VerifyThePhoneNumberValidationWhileBookingARideInConciergePortals_Main extends TestBase {
	...
	public DEV_TC_1693_VerifyThePhoneNumberValidationWhileBookingARideInConciergePortals_Main(WebDriver driver) {
		try {
			this.driver = driver;
	...
	public Boolean verifyRideBookingConfirmation(Boolean visibilityStatus) {
		try {
			...
			objTestBase.defaultWaitTime(1000);
			if (displayStatus.isDisplayed() && msgText.contains("please"))
				visibilityStatus = true;
			else
				visibilityStatus = false;
