...
public class DEV_TC_1680_VerifyDisplayingOfCorporateNameInDispatchPortalRideExpandViewWhenRideBookedFromConciergePortal_Main
		extends TestBase {
	...
	@FindBy(xpath = "//button[normalize-space()='Sign In']")
	WebElement signInBtn_LoginDispatch;

	@FindBy(xpath = "//button[@class='accoutBtn dropdown-toggle']")
	WebElement signInBtnDropdownDispatch;

	@FindBy(xpath = "(//span[normalize-space()='New Booking'])[1]")
	WebElement newBookingTabDispatch;
	...
	WebElement searchUserDispatch;

	@FindBy(xpath = "//label[normalize-space()= 'Booking User Information']")
	List<WebElement> bookingUserInfoSection;

	@FindBy(xpath = "//b[normalize-space()='Corporate Company Name:']")
	List<WebElement> companyName;

	@FindBy(xpath = "(//*[normalize-space()='Company Name:'])//text()")
	List<WebElement> companyNameText;
	...
	public DEV_TC_1680_VerifyDisplayingOfCorporateNameInDispatchPortalRideExpandViewWhenRideBookedFromConciergePortal_Main(
			WebDriver driver) {
		try {
			this.driver = driver;
	...
	public Boolean visibilityOfSigninButtonDispatch(Boolean visibilityStatus) {
		try {
			if (signInBtn_LoginDispatch.isDisplayed())
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception ex) {
	...
	public Boolean visibilityOfLoggedinUserDispatch(Boolean visibilityStatus) {
		try {
			objTestBase.defaultWaitTime(2000);
			if (signInBtnDropdownDispatch.isDisplayed()) {
				expected = signInBtnDropdownDispatch.getText();
				if (expected.toLowerCase().contains("welcome")) {
					visibilityStatus = true;
				} else {
					visibilityStatus = false;
	...
	public Boolean visibilitOfNewBookingTabDispatch(Boolean visibilityStatus) {
		try {
			action = new Actions(driver);
			action.moveToElement(newBookingTabDispatch).click().build().perform();
			objTestBase.defaultWaitTime(3000);
			expected = driver.getCurrentUrl();
	...
	public void clickRideAllDispatch() {
		try {
			ridesTabAllDispatch.click();
			objTestBase.defaultWaitTime(5000);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public Boolean searchTripIDDispatch(Boolean visibilityStatus, String tripID) {
		try {
			action = new Actions(driver);
			WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(20));
			WebElement displayStatus = wait.until(ExpectedConditions.visibilityOf(searchUserDispatch));
			if (displayStatus.isDisplayed()) {
				searchUserDispatch.click();
				searchUserDispatch.sendKeys(Keys.CONTROL + "A");
				searchUserDispatch.sendKeys(Keys.DELETE);
				objTestBase.defaultWaitTime(1000);
				action.moveToElement(searchUserDispatch).sendKeys(tripID).build().perform();
				objTestBase.defaultWaitTime(3000);
				if (searchUserDispatch.isDisplayed())
					visibilityStatus = true;
				else
					visibilityStatus = false;
			} else
	...
	public Boolean verifyCorporateCompanyNameInExpandedView(Boolean visibilityStatus) {
		try {
			action = new Actions(driver);
			...
			if (bookingUserInfoSection.size() != 0) {
				action.moveToElement(bookingUserInfoSection.get(0)).click().build().perform();
				objTestBase.defaultWaitTime(1000);
				for (int i = 0; i < 2; i++) {
					objTestBase.defaultWaitTime(500);
					action.sendKeys(Keys.DOWN).build().perform();
