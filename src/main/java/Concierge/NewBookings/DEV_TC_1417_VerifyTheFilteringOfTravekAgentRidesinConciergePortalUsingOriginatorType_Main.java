...
public class DEV_TC_1417_VerifyTheFilteringOfTravekAgentRidesinConciergePortalUsingOriginatorType_Main
		extends TestBase {
	...
	public DEV_TC_1417_VerifyTheFilteringOfTravekAgentRidesinConciergePortalUsingOriginatorType_Main(WebDriver driver) {
		try {
			this.driver = driver;
	...
	public Boolean visibilityOfOriginatorDropSection(Boolean visibilityStatus) {
		try {
			...
			select.selectByValue("travel_arranger");
			objTestBase.defaultWaitTime(3000);
	...
	public void enterSearch() {
		try {
			searchUser.sendKeys(prop.getProperty("conciergeFName"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public Boolean validateConciergerideDetails(Boolean visibilityStatus, String scenario) {
		try {
			if (sourceValue.size() != 0) {
				...
				travelAgentTripIDsOriginator.put("travelAgent_" + scenario, tripID);
				...
				if (sourceValue.size() != 0) {
					...
					visibilityStatus = false;
					travelAgentTripIDsOriginator.put("travelAgent_" + scenario, "No_Data");
				}
			} else {
