...
public class DEV_TC_1649_VerifyTheFunctionalityOfHomeRequestNewQuoteCancelQuoteInEventAndSpecialServiceQuoteRequestPageInConciergePortal_Main
		extends TestBase {
	WebDriver driver;
	Actions action;
	String expected;
	...
	@FindBy(xpath = "(//button[normalize-space()='CANCEL QUOTE'])[1]")
	...
	public DEV_TC_1649_VerifyTheFunctionalityOfHomeRequestNewQuoteCancelQuoteInEventAndSpecialServiceQuoteRequestPageInConciergePortal_Main(
			WebDriver driver) {
		try {
			this.driver = driver;
	...
	public Boolean visibilityOfColoumnsInManageEventsPage(Boolean visibilityStatus) {
		try {
			if (actionColumn.isDisplayed() && qouteColumn.isDisplayed() && pickupColumn.isDisplayed()
					&& dateColumn.isDisplayed() && dropColumn.isDisplayed() && passengerscolumn.isDisplayed())
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception ex) {
	...
	public void clickOnCreateQouteBtn() {
		try {
			action = new Actions(driver);
			objTestBase.defaultWaitTime(1000);
			action.moveToElement(createQouteBtn).click().build().perform();
			objTestBase.defaultWaitTime(3000);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public Boolean visibilityOfCreateQoutePage(Boolean visibilityStatus) {
		try {
			if (eventQoute.isDisplayed())
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception ex) {
	...
	public void clickOnRequestNewQouteBtn() {
		try {
			action = new Actions(driver);
			objTestBase.defaultWaitTime(1000);
			action.moveToElement(requestNewQouteBtn).click().build().perform();
			objTestBase.defaultWaitTime(3000);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public Boolean visibilityOfpopup(Boolean visibilityStatus) {
		try {
