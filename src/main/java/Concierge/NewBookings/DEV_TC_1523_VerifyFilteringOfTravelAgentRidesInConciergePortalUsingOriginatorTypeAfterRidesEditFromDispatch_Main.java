...
public class DEV_TC_1523_VerifyFilteringOfTravelAgentRidesInConciergePortalUsingOriginatorTypeAfterRidesEditFromDispatch_Main
		extends TestBase {
	...
	@FindBy(xpath = "//div[@title='Travel Arranger']")
	List<WebElement> sourceValueConcierge;

	public DEV_TC_1523_VerifyFilteringOfTravelAgentRidesInConciergePortalUsingOriginatorTypeAfterRidesEditFromDispatch_Main(
			WebDriver driver) {
		try {
			this.driver = driver;
...
//					"2FA Validation Status for Script - " + getClass().getSimpleName() + " = " + visibilityStatus1);
	...
	public Boolean visibilityOfRespectivePage(Boolean visibilityStatus, String scenario) {
		try {
			...
			if (scenario.contains("travelAgent_Rides_All") && expected.toLowerCase().contains("all"))
				visibilityStatus = true;
			else if (scenario.contains("travelAgent_InProgress") && expected.toLowerCase().contains("inprogress"))
				visibilityStatus = true;
			else if (scenario.contains("travelAgent_Upcoming") && expected.toLowerCase().contains("upcoming"))
				visibilityStatus = true;
			else if (scenario.contains("travelAgent_Available") && expected.toLowerCase().contains("available"))
				visibilityStatus = true;
	...
	public String validateTripIDDetails(String scenario, String ridePickupDate) {
		...
		if (visibilityStatus.booleanValue() == true) {
			if (!scenario.equalsIgnoreCase("travelAgent_Available")
				...
				visibilityStatus = true;
			} else if (scenario.equalsIgnoreCase("travelAgent_Available")
					&& (!rideStatus.getText().toLowerCase().contains("cancelled")
							|| !rideStatus.getText().toLowerCase().contains("completed"))
	...
	public void Originator() {
		try {
			...
			select.selectByValue(prop.getProperty("originatorDropdownTa"));
			objTestBase.defaultWaitTime(2000);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public Boolean verifySourceSystemConcierge(Boolean visibilityStatus) {
		try {
			if (sourceValueConcierge.size() != 0) {
				...
				if (expected.equalsIgnoreCase("travel arranger"))
					visibilityStatus = true;
				else
					visibilityStatus = false;
			}
