...
public class DEV_TC_1901_VerifyNewBookingFunctionalityWithGOOGLEPAYUnderAvailaiblePaymentMethodsForConciergePortal_Main
		extends TestBase {
	...
	@FindBy(xpath = "//a[normalize-space()='Point to Point']")
	WebElement pointToPointTab;
	...
	WebElement payingWithgooglePayOption;

	@FindBy(xpath = "//button[@aria-label='Google Pay']")
	WebElement googlePayOption;
	...
	public DEV_TC_1901_VerifyNewBookingFunctionalityWithGOOGLEPAYUnderAvailaiblePaymentMethodsForConciergePortal_Main(
			WebDriver driver) {
		try {
			this.driver = driver;
	...
	public void clickPointToPointTab() {
		try {
			action = new Actions(driver);
			objTestBase.defaultWaitTime(1000);
			action.moveToElement(pointToPointTab).click().build().perform();
			objTestBase.defaultWaitTime(1000);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public Boolean verifyGooglePayOptionVisibility(Boolean visibilityStatus) {
		try {
			if (payingWithgooglePayOption.isDisplayed())
				visibilityStatus = true;
			else
				visibilityStatus = false;
		} catch (Exception e) {
	...
	public void clickOnpayingWithgooglePayOption() {
		try {
			payingWithgooglePayOption.click();
			defaultWaitTime(3000);
			googlePayOption.click();
			defaultWaitTime(3000);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public Boolean verifyContinueBtnVisibility(String  parentWindow, Boolean visibilityStatus) {
		try {
			...
			while (windowsIterator.hasNext()) {
				...
				if (!parentWindow.equals(child_window)) {
					...
					driver.switchTo().frame("sM432dIframe");
