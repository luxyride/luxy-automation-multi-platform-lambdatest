...
public class DEV_TC_1902_VerifyNewBookingFunctionalityWithPayPalUnderAvailaiblePaymentMethodsForConciergePortal_Main
		extends TestBase {
	...
	@FindBy(xpath = "//a[normalize-space()='Hourly']")
	...
	@FindBy(xpath = "//span[normalize-space()='Checkout']//preceding::img[@aria-label='paypal']")	
	WebElement paypalPayCheckoutOption;
	...
	public DEV_TC_1902_VerifyNewBookingFunctionalityWithPayPalUnderAvailaiblePaymentMethodsForConciergePortal_Main(
			WebDriver driver) {
		try {
			this.driver = driver;
	...
	public Boolean verifyPaypalCheckoutBtnVisibility(String parentWindow, Boolean visibilityStatus) {
		try {
	...
	public void enterPayPalEmailID() {
		try {
			paypalEMail.sendKeys(prop.getProperty("paypalEMailID"));
			defaultWaitTime(1000);
			nextButton.click();
	...
	public void enterPayPalPassword() {
		try {
			paypalPassword.sendKeys(prop.getProperty("paypalPassword"));
			defaultWaitTime(1000);
		} catch (Exception e) {
			e.printStackTrace();
		}
