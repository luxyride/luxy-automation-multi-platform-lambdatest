...
public class DEV_TC_1499_VerifyTheDisplayingOfInternalNotesInEditRideModeAfterBookingARideInConciergePortal_Main
		extends TestBase {
	...
	public DEV_TC_1499_VerifyTheDisplayingOfInternalNotesInEditRideModeAfterBookingARideInConciergePortal_Main(
			WebDriver driver) {
		try {
			this.driver = driver;
	...
	public Boolean searchPassenger(Boolean visibilityStatus) {
		try {
			action = new Actions(driver);
			objTestBase.defaultWaitTime(1000);
			action.moveToElement(searchPassenger).click().sendKeys(prop.getProperty("conciergeadminMail")).build()
					.perform();
		...
		try {
			...
			if (expected.toLowerCase().contains("all")) {
				...
				if (bookingRideTripIdConcierge.isDisplayed()) {
					...
					System.out.println(bookingRideTripIdConcierge.getText());
					...
					if ((!rideStatusConcierge.getText().toLowerCase().contains("cancelled")
							...
							&& bookedPassengerNameConcierge.getText().toLowerCase().contains("automation")
					...
					utillLogger.info(
							"DEV_TC_1499_VerifyTheDisplayingOfInternalNotesInEditRideModeAfterBookingARideInConciergePortal_Main - Capture the Trip ID = "
									+ tripID);
					visibilityStatus = true;
				} else
					visibilityStatus = false;
	...
	public void enterFirstName() {
		try {
			...
			firstName.click();
			objTestBase.defaultWaitTime(1000);
			action.moveToElement(firstName).click().sendKeys(prop.getProperty("conciergeadminMail")).build().perform();
			objTestBase.defaultWaitTime(3000);
			action.sendKeys(Keys.TAB).build().perform();
			objTestBase.defaultWaitTime(3000);
			action.sendKeys(Keys.ENTER).build().perform();
	...
	public Boolean verifyInternalNotesExpandedView(Boolean visibilityStatus, String scenario) {
		try {
			action.moveToElement(bookingRideTripIdConcierge).click().build().perform();
			objTestBase.defaultWaitTime(2000);
