...
public class DEV_TC_1879_VerifyTheSortingFunctionalityOfBucketNameForAffiliatesInDispatchPortal_Test
		extends TestBase {
	...
	DEV_TC_1879_VerifyTheSortingFunctionalityOfBucketNameForAffiliatesInDispatchPortal_Main objVerifySighninMain;
	DEV_TC_1879_VerifyTheSortingFunctionalityOfBucketNameForAffiliatesInDispatchPortal_Test objVerifySighninTest;
	...
	public void VerifySignInWithValidCredentials_Test() throws Exception {
		...
		try {
			...
			objVerifySighninMain = new DEV_TC_1879_VerifyTheSortingFunctionalityOfBucketNameForAffiliatesInDispatchPortal_Main(
					driver);
			objVerifySighninTest = new DEV_TC_1879_VerifyTheSortingFunctionalityOfBucketNameForAffiliatesInDispatchPortal_Test();
			objTestBase = new TestBase();
			...
			if (testStatus == "PASSED") {
				...
				if (testStatus == "PASSED") {
					testStep = "Verification of Bucket Name column in Manage Affiliate Page";
					visibilityStatus = objVerifySighninMain.verifyBucketNameInManageAffiliatePage(visibilityStatus);
					if (visibilityStatus.booleanValue() == true) {
						if (localExecutionFlag == true)	objupdateResults.updateResults(screenshotPath, logger, LogStatus.PASS, testStep, exception);	else	lambdaTestStatusUpdate("passed", testStep);
						testStatus = "PASSED";
					} else {
