...
public class DEV_TC_1926_VerifyFullRefundWhenTheRideIsCanceledInConditionsLikeNoAvailabilityDriverNoShowAndSuspectedFraud_Test
		extends TestBase {
	...
	DEV_TC_1926_VerifyFullRefundWhenTheRideIsCanceledInConditionsLikeNoAvailabilityDriverNoShowAndSuspectedFraud_Main objVerifyNewBookingDispatchMain;
	DEV_TC_1926_VerifyFullRefundWhenTheRideIsCanceledInConditionsLikeNoAvailabilityDriverNoShowAndSuspectedFraud_Test objVerifyNewBookingDispatchTest;
	...
	public void verificationOfNewBookingFromDispatchSite() throws Exception {
		...
		try {
			...
			objVerifyNewBookingDispatchMain = new DEV_TC_1926_VerifyFullRefundWhenTheRideIsCanceledInConditionsLikeNoAvailabilityDriverNoShowAndSuspectedFraud_Main(
					driver);
			objVerifyNewBookingDispatchTest = new DEV_TC_1926_VerifyFullRefundWhenTheRideIsCanceledInConditionsLikeNoAvailabilityDriverNoShowAndSuspectedFraud_Test();
			objTestBase = new TestBase();
	...
	public String vechileBooking(String scenario, String testStatus) throws Exception {
		try {
			...
			testStep = "Verification of Rides All Page loaded for";
			visibilityStatus = objVerifyNewBookingDispatchMain.visibilityOfRidesAllPage(visibilityStatus);
			if (visibilityStatus.booleanValue() == true) {
				if (localExecutionFlag == true)
					objupdateResults.updateResults(screenshotPath, logger, LogStatus.PASS, testStep, exception);
				else
			...
			objTestBase.defaultWaitTime(1000);
			objVerifyNewBookingDispatchMain.searchRideId(tripID);
			objTestBase.defaultWaitTime(5000);

			testStep = "visibility Of Cancel button under Actions Column";
			visibilityStatus = objVerifyNewBookingDispatchMain.visibilityOfCancelButton(visibilityStatus);
			if (visibilityStatus.booleanValue() == true) {
				if (localExecutionFlag == true)
					objupdateResults.updateResults(screenshotPath, logger, LogStatus.PASS, testStep, exception);
				else
			...
			objTestBase.defaultWaitTime(1000);
			objVerifyNewBookingDispatchMain.clickOnCancelBtn();
			objTestBase.defaultWaitTime(2000);

			testStep = "Verification of Cancelling Ride Booking ";
			visibilityStatus = objVerifyNewBookingDispatchMain.visibilityOfReviewCancleRidepopup(visibilityStatus);
			if (visibilityStatus.booleanValue() == true) {
				if (localExecutionFlag == true)
					objupdateResults.updateResults(screenshotPath, logger, LogStatus.PASS, testStep, exception);
				else
			...
			objTestBase.defaultWaitTime(2000);
			if (testStatus == "PASSED") {
				visibilityStatus = objVerifyNewBookingDispatchMain.visibilityOfConfirmCancleRidepopup(visibilityStatus);
				if (visibilityStatus.booleanValue() == true) {
					objupdateResults.updateResults(screenshotPath, logger, LogStatus.PASS, testStep, exception);
					testStatus = "PASSED";
				} else {
			...
			if (testStatus == "PASSED") {
				visibilityStatus = objVerifyNewBookingDispatchMain.visibilityOfCancelRideConfirmationMsg(visibilityStatus);
				if (visibilityStatus.booleanValue() == true) {
					objupdateResults.updateResults(screenshotPath, logger, LogStatus.PASS, testStep, exception);
					testStatus = "PASSED";
				} else {
