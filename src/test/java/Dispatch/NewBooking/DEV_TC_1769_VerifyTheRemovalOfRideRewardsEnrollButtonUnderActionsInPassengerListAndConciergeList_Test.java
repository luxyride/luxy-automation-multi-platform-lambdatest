...
public class DEV_TC_1769_VerifyTheRemovalOfRideRewardsEnrollButtonUnderActionsInPassengerListAndConciergeList_Test
		extends TestBase {
	...
	DEV_TC_1769_VerifyTheRemovalOfRideRewardsEnrollButtonUnderActionsInPassengerListAndConciergeList_Main objVerifySighninMain;
	DEV_TC_1769_VerifyTheRemovalOfRideRewardsEnrollButtonUnderActionsInPassengerListAndConciergeList_Test objVerifySighninTest;
	...
	public void VerifySignInWithValidCredentials_Test() throws Exception {
		...
		try {
			...
			objVerifySighninMain = new DEV_TC_1769_VerifyTheRemovalOfRideRewardsEnrollButtonUnderActionsInPassengerListAndConciergeList_Main(
					driver);
			objVerifySighninTest = new DEV_TC_1769_VerifyTheRemovalOfRideRewardsEnrollButtonUnderActionsInPassengerListAndConciergeList_Test();
			objTestBase = new TestBase();
			...
			if (testStatus == "PASSED") {
				...
				objTestBase.defaultWaitTime(2000);
				objVerifySighninMain.clickOnPassengers();
				objTestBase.defaultWaitTime(3000);

				testStep = "Verification of Removal Enroll button for all passengers list ";
				visibilityStatus = objVerifySighninMain.verifyOfEnrollButtonShouldNotBeAppear(visibilityStatus);
				if (visibilityStatus.booleanValue() == true) {
					if (localExecutionFlag == true)
						objupdateResults.updateResults(screenshotPath, logger, LogStatus.PASS, testStep, exception);
					else
				...
				testStep = "Verification of Removal Enroll button for all Concierge List In Dispatch ";
				visibilityStatus = objVerifySighninMain.verifyOfEnrollButtonShouldNotBeAppear(visibilityStatus);
				if (visibilityStatus.booleanValue() == true) {
					if (localExecutionFlag == true)
						objupdateResults.updateResults(screenshotPath, logger, LogStatus.PASS, testStep, exception);
