...
public class DEV_TC_1767_VerifyRemovalOfActionColumnUnderSettingsCoverageZonesInDispatch_Test extends TestBase {
	...
	DEV_TC_1767_VerifyRemovalOfActionColumnUnderSettingsCoverageZonesInDispatch_Main objVerifySighninMain;
	DEV_TC_1767_VerifyRemovalOfActionColumnUnderSettingsCoverageZonesInDispatch_Test objVerifySighninTest;
	...
	public void VerifySignInWithValidCredentials_Test() throws Exception {
		...
		try {
			...
			objVerifySighninMain = new DEV_TC_1767_VerifyRemovalOfActionColumnUnderSettingsCoverageZonesInDispatch_Main(
					driver);
			objVerifySighninTest = new DEV_TC_1767_VerifyRemovalOfActionColumnUnderSettingsCoverageZonesInDispatch_Test();
			objTestBase = new TestBase();
			...
			if (testStatus == "PASSED") {
				...
				objTestBase.defaultWaitTime(2000);
				objVerifySighninMain.clickOnSettingsDropDown();
				objTestBase.defaultWaitTime(3000);

				testStep = "verification of dropwdown list in settings";
				visibilityStatus = objVerifySighninMain.verifySettingsDropDownList(visibilityStatus);
				if (visibilityStatus.booleanValue() == true) {
					if (localExecutionFlag == true)
						objupdateResults.updateResults(screenshotPath, logger, LogStatus.PASS, testStep, exception);
					else
				...
				objTestBase.defaultWaitTime(2000);
				objVerifySighninMain.clickOnCoverageZones();
				objTestBase.defaultWaitTime(3000);

				testStep = "visibility of coloumns in coverage zones Page";
				visibilityStatus = objVerifySighninMain.verifycoloumnsInCoverageZonePage(visibilityStatus);
				if (visibilityStatus.booleanValue() == true) {
					if (localExecutionFlag == true)
						objupdateResults.updateResults(screenshotPath, logger, LogStatus.PASS, testStep, exception);
					else
