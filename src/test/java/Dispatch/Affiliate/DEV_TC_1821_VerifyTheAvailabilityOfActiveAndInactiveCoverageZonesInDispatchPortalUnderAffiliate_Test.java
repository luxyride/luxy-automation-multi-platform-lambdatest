...
public class DEV_TC_1821_VerifyTheAvailabilityOfActiveAndInactiveCoverageZonesInDispatchPortalUnderAffiliate_Test
		extends TestBase {
	...
	DEV_TC_1821_VerifyTheAvailabilityOfActiveAndInactiveCoverageZonesInDispatchPortalUnderAffiliate_Main objVerifySighninMain;
	DEV_TC_1821_VerifyTheAvailabilityOfActiveAndInactiveCoverageZonesInDispatchPortalUnderAffiliate_Test objVerifySighninTest;
	...
	public void VerifySignInWithValidCredentials_Test() throws Exception {
		...
		try {
			...
			objVerifySighninMain = new DEV_TC_1821_VerifyTheAvailabilityOfActiveAndInactiveCoverageZonesInDispatchPortalUnderAffiliate_Main(
					driver);
			objVerifySighninTest = new DEV_TC_1821_VerifyTheAvailabilityOfActiveAndInactiveCoverageZonesInDispatchPortalUnderAffiliate_Test();
			objTestBase = new TestBase();
			...
			if (testStatus == "PASSED") {
				...
				if (testStatus == "PASSED") {
					testStep = "Verification of Searching the Affiliate";
					visibilityStatus = objVerifySighninMain.searchAffiliate(visibilityStatus);
					if (visibilityStatus.booleanValue() == true) {
						if (localExecutionFlag == true)
							objupdateResults.updateResults(screenshotPath, logger, LogStatus.PASS, testStep, exception);
						else
				...
				if (testStatus == "PASSED") {
					testStep = "Verification of Affiliate EDIT Page Navigation and visibility of Zones";
					visibilityStatus = objVerifySighninMain.editAffiliatePageNavigation(visibilityStatus);
					if (visibilityStatus.booleanValue() == true) {
						if (localExecutionFlag == true)
							objupdateResults.updateResults(screenshotPath, logger, LogStatus.PASS, testStep, exception);
						else
