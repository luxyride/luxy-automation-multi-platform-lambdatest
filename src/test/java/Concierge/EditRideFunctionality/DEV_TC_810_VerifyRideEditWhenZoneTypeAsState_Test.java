...
public class DEV_TC_810_VerifyRideEditWhenZoneTypeAsState_Test extends TestBase {
	...
	DEV_TC_810_VerifyRideEditWhenZoneTypeAsState_Main objVerifyEditRidesZone_Main;
	DEV_TC_810_VerifyRideEditWhenZoneTypeAsState_Test objVerifyEditRidesZone_Test;
	...
	public void verifyRideEditWhenZoneTypeAsState() throws Exception {
		try {
			...
			objVerifyEditRidesZone_Main = new DEV_TC_810_VerifyRideEditWhenZoneTypeAsState_Main(driver);
			objVerifyEditRidesZone_Test = new DEV_TC_810_VerifyRideEditWhenZoneTypeAsState_Test();
			objTestBase = new TestBase();
			...
			// Verification of Ride Booking Details - START:
			scenario = "Book a Ride";
			tripID = vechileBooking(scenario, testStatus, tripID);
			System.out.println("Trip ID = " + tripID);
			// Verification of Ride Booking Details - END:

			// Verification of BufferTime Scenarios Validations - START:
			testStep = "Verification of BufferTime Scenarios Validations";
			if (!tripID.isEmpty()) {
				...
				if (testStatus == "PASSED") {
					...
					if (testStatus == "PASSED") {
						...
						objVerifyEditRidesZone_Main.clickOnUpdateBtn();
						objTestBase.defaultWaitTime(1000);
			...
			if (testStatus == "PASSED" && !tripID.isEmpty() && !environmentCode.equalsIgnoreCase("prod")) {
				...
				if (enableDONOTSERVE == true) {
					...
					testStep = "Verify the EDIT Ride Details Page - After Do Not Serve Enabled";
					visibilityStatus = objVerifyEditRidesZone_Main.validateEDITRideBookingPage(visibilityStatus);
					if (visibilityStatus.booleanValue() == true) {
						if (localExecutionFlag == true)	objupdateResults.updateResults(screenshotPath, logger, LogStatus.PASS, testStep, exception);	else	lambdaTestStatusUpdate("passed", testStep);
						testStatus = "PASSED";
		...
		try {
			...
			if (!environmentCode.equalsIgnoreCase("prod")) {
				...
				if (testStatus == "PASSED") {
					...
					System.out.println("Trip ID = " + tripID);
					testStep = "Verification of Capturing the Ride ID for SEDAN Vehicle - Trip ID = " + tripID;
					if (tripID != null)
						if (localExecutionFlag == true)	objupdateResults.updateResults(screenshotPath, logger, LogStatus.PASS, testStep, exception);	else	lambdaTestStatusUpdate("passed", testStep);
					else
						if (localExecutionFlag == true)	objupdateResults.updateResults(screenshotPath, logger, LogStatus.FAIL, testStep, exception);	else	lambdaTestStatusUpdate("failed", testStep);
	...
	public void afterClass() {
		...
		for (Map.Entry<String, String> entry : rideBookingIdsConcierge.entrySet()) {
			System.out.println(
					"EditRideFunctionality Ride IDs from TC: DEV_TC_810_VerifyRideEditWhenZoneTypeAsState_Test = "
							+ "Total TripIDs = " + rideBookingIdsConcierge.size() + " - " + entry.getKey() + " - "
							+ entry.getValue());
			utillLogger
					.info("EditRideFunctionality Ride IDs from TC: DEV_TC_810_VerifyRideEditWhenZoneTypeAsState_Test = "
							+ "Total TripIDs = " + rideBookingIdsConcierge.size() + " - " + entry.getKey() + " - "
							+ entry.getValue());
		}
		utillLogger.info("//--------------------------------------------------------------------------------------//");
