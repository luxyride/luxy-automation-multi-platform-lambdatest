...
public class DEV_TC_1811_VerifyRemoveOrderIdFromConciergePortal_Test extends TestBase {
	...
	DEV_TC_1811_VerifyRemoveOrderIdFromConciergePortal_Main objVerifyNewBookingConciergeMain;
	DEV_TC_1811_VerifyRemoveOrderIdFromConciergePortal_Test objVerifyNewBookingConciergeTest;
	...
	public void verificationOfNewBookingFromDispatchSite() throws Exception {
		...
		try {
			...
			objVerifyNewBookingConciergeMain = new DEV_TC_1811_VerifyRemoveOrderIdFromConciergePortal_Main(driver);
			objVerifyNewBookingConciergeTest = new DEV_TC_1811_VerifyRemoveOrderIdFromConciergePortal_Test();
			objTestBase = new TestBase();
	...
	public String vechileBooking(String scenario, String testStatus) throws Exception {
		try {
			...
			visibilityStatus = objVerifyNewBookingConciergeMain.searchTripID(tripID);
			if (visibilityStatus.booleanValue() == true) {
				if (localExecutionFlag == true)
					objupdateResults.updateResults(screenshotPath, logger, LogStatus.PASS, testStep, exception);
				else
			...
			if (testStatus == "PASSED") {
				...
				visibilityStatus = objVerifyNewBookingConciergeMain.verifyOrderId(visibilityStatus, scenario);
				if (visibilityStatus.booleanValue() == true) {
					if (localExecutionFlag == true)
						objupdateResults.updateResults(screenshotPath, logger, LogStatus.PASS, testStep, exception);
					else
				...
				if (testStatus == "PASSED") {
					testStatus = " ";
					objVerifyNewBookingConciergeMain.affiliateeMailInput();
					objTestBase.defaultWaitTime(1000);
					objVerifyNewBookingConciergeMain.affiliatePasswordInput();
					objTestBase.defaultWaitTime(1000);
					objVerifyNewBookingConciergeMain.affiliateEyeIconClick();
					objTestBase.defaultWaitTime(1000);
					...
					if (visibilityStatus.booleanValue() == true) {
						if (localExecutionFlag == true)
							...
							lambdaTestStatusUpdate("passed", testStep);
						objVerifyNewBookingConciergeMain.affiliateClickSigninButton();
						objTestBase.defaultWaitTime(3000);
						driver.navigate().refresh();
						objTestBase.defaultWaitTime(1000);
						testStatus = "PASSED";
					...
					objTestBase.defaultWaitTime(2000);
					objVerifyNewBookingConciergeMain.affiliateClickOnRides();
					objTestBase.defaultWaitTime(3000);
					...
					objTestBase.defaultWaitTime(2000);
					objVerifyNewBookingConciergeMain.affiliateClickOnAvailableTab();
					objTestBase.defaultWaitTime(3000);
					objVerifyNewBookingConciergeMain.searchTripID(tripID);
					objTestBase.defaultWaitTime(2000);
		...
		try {
			...
			if (tripID != null) {
				...
				if (testStatus == "PASSED") {
					...
					objTestBase.defaultWaitTime(2000);
					objVerifyNewBookingConciergeMain.clickOnFarmOut();
					objTestBase.defaultWaitTime(2000);
					objVerifyNewBookingConciergeMain.enterSearchInput();
					objTestBase.defaultWaitTime(5000);
					objVerifyNewBookingConciergeMain.clickOnCheckBox();
					objTestBase.defaultWaitTime(2000);

					testStep = "Verification of success Message after Farming out a Ride";
					visibilityStatus = objVerifyNewBookingConciergeMain.visibilityOfSuccessMessage(visibilityStatus);
					if (visibilityStatus.booleanValue() == true) {
						if (localExecutionFlag == true)
							objupdateResults.updateResults(screenshotPath, logger, LogStatus.PASS, testStep, exception);
						else
