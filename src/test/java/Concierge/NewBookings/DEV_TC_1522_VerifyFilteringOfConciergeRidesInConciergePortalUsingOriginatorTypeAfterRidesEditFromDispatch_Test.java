...
public class DEV_TC_1522_VerifyFilteringOfConciergeRidesInConciergePortalUsingOriginatorTypeAfterRidesEditFromDispatch_Test
		extends TestBase {
	...
	DEV_TC_1522_VerifyFilteringOfConciergeRidesInConciergePortalUsingOriginatorTypeAfterRidesEditFromDispatch_Main objVerifyEditDetails_Main;
	DEV_TC_1522_VerifyFilteringOfConciergeRidesInConciergePortalUsingOriginatorTypeAfterRidesEditFromDispatch_Test objVerifyEditDetails_Test;
	...
	public void verificationOfRideEditDetailsFromDispatchSite() throws Exception {
		try {
			...
			objVerifyEditDetails_Main = new DEV_TC_1522_VerifyFilteringOfConciergeRidesInConciergePortalUsingOriginatorTypeAfterRidesEditFromDispatch_Main(
					driver);
			objVerifyEditDetails_Test = new DEV_TC_1522_VerifyFilteringOfConciergeRidesInConciergePortalUsingOriginatorTypeAfterRidesEditFromDispatch_Test();
			objTestBase = new TestBase();
			...
			if (!conciergeTripIDsOriginator.isEmpty()) {
				...
				for (Map.Entry<String, String> entry : conciergeTripIDsOriginator.entrySet()) {
					testStatus = "";
					testStep = "Verification of Update Trip Details from Dispatch.";
					scenario = entry.getKey();
					tripID = entry.getValue();
					if (tripID != "No_Data") {
						if ((!scenario.equalsIgnoreCase("concierge_Cancelled"))
								&& (!scenario.equalsIgnoreCase("concierge_Completed"))) {

							objVerifyEditDetails_Main.clickRidesSection();
							objTestBase.defaultWaitTime(3000);

							if (entry.getKey().contains("concierge_Rides_All"))
								objVerifyEditDetails_Main.clickonAllTab();
							else if (entry.getKey().contains("concierge_InProgress"))
								objVerifyEditDetails_Main.clickonInProgressTab();
							else if (entry.getKey().contains("concierge_Upcoming"))
								objVerifyEditDetails_Main.clickonUpcomingTab();
							else if (entry.getKey().contains("concierge_Available"))
				...
				if (!conciergeTripIDsOriginator.isEmpty()) {
					...
					for (Map.Entry<String, String> entry : conciergeTripIDsOriginator.entrySet()) {
						...
						if (tripID != "No_Data") {
							if (scenario != "concierge_Cancelled" && scenario != "concierge_Completed")
							...
							if (scenario.contains("concierge_Rides_All"))
								objVerifyEditDetails_Main.clickonAllTab();
							else if (scenario.contains("concierge_InProgress"))
								objVerifyEditDetails_Main.clickonInProgressTab();
							else if (scenario.contains("concierge_Upcoming"))
								objVerifyEditDetails_Main.clickonUpcomingTab();
							else if (scenario.contains("concierge_Available"))
								objVerifyEditDetails_Main.clickonAvailableTab();
					...
					objupdateResults.updateResults(screenshotPath, logger, LogStatus.SKIP, testStep + " - No Data",
							exception);
			} else
	...
	public void afterClass() {
		...
		for (Map.Entry<String, String> entry : conciergeTripIDsOriginator.entrySet()) {
			System.out.println(
					"EDIT Ride IDs from TC: DEV_TC_1522_VerifyFilteringOfConciergeRidesInConciergePortalUsingOriginatorTypeAfterRidesEditFromDispatch_Test = "
							+ "Total TripIDs = " + conciergeTripIDsOriginator.size() + " - " + entry.getKey() + " - "
							+ entry.getValue());
