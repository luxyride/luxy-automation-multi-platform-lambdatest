...
public class DEV_TC_1778_HourlyVerifyBookingARideInHourlyTabConciergeByEnablingSameAsPickUpAddressCheckBox_Test extends TestBase {
	...
	DEV_TC_1778_HourlyVerifyBookingARideInHourlyTabConciergeByEnablingSameAsPickUpAddressCheckBox_Main objVerifyNewBookingConciergeMain;
	DEV_TC_1778_HourlyVerifyBookingARideInHourlyTabConciergeByEnablingSameAsPickUpAddressCheckBox_Test objVerifyNewBookingConciergeTest;
	...
	public void beforeClass(String browser) {
		try {
			...
			// Clear the conciergehourlyRideTripIDs which were booked from DEV_TC_1126:
	...
	public void verificationOfNewBookingFromDispatchSite() throws Exception {
		...
		try {
			...
			objVerifyNewBookingConciergeMain = new DEV_TC_1778_HourlyVerifyBookingARideInHourlyTabConciergeByEnablingSameAsPickUpAddressCheckBox_Main(
					driver);
			objVerifyNewBookingConciergeTest = new DEV_TC_1778_HourlyVerifyBookingARideInHourlyTabConciergeByEnablingSameAsPickUpAddressCheckBox_Test();
			objTestBase = new TestBase();
		...
		try {
			...
			if (!environmentCode.equalsIgnoreCase("prod")) {
				...
				if (testStatus == "PASSED") {
					...
					conciergehourlyRideTripID = objVerifyNewBookingConciergeMain.captureRideBookingIDs(visibilityStatus, scenario, conciergehourlyRideTripID);
					System.out.println("Trip ID = " + conciergehourlyRideTripID);
					testStep = "Verification of Capturing the Ride ID for " + scenario + " - Trip ID = " + conciergehourlyRideTripID;
					if (conciergehourlyRideTripID != null)
						if (localExecutionFlag == true)
							objupdateResults.updateResults(screenshotPath, logger, LogStatus.PASS, testStep, exception);
						else
							lambdaTestStatusUpdate("passed", testStep);
	...
	private Boolean validateActionsIconVisibilityFromDispatch(Boolean visibilityStatus, String scenario,
			String conciergehourlyRideTripID) {
		try {
			...
			driver.navigate().refresh();
			objTestBase.defaultWaitTime(3000);
			objVerifyNewBookingConciergeMain.searchRideIdDispatch(conciergehourlyRideTripID);
			objTestBase.defaultWaitTime(5000);

			// Validate the conciergehourlyRideTripID Details:
			testStep = "Verify the RideStatus, PassengerName & Ride Booked Date for " + scenario.toUpperCase()
					+ " - Trip ID: " + conciergehourlyRideTripID;
			visibilityStatus = objVerifyNewBookingConciergeMain.validateconciergehourlyRideTripIDDetailsDispatch(visibilityStatus);
			if (visibilityStatus.booleanValue() == true) {
				if (localExecutionFlag == true)
					objupdateResults.updateResults(screenshotPath, logger, LogStatus.PASS, testStep, exception);
				else
			...
			testStep = "Verify the ACTIONs Icon is in Disabled status for the - Trip ID: " + conciergehourlyRideTripID;
			if (testStatus == "PASSED") {
		...
		for (Map.Entry<String, String> entry : rideBookingIdsConcierge.entrySet()) {
			System.out
					.println("Booking IDs from TC: DEV_TC_1248_VerifyNewRideBookingFunctionalityFromConciergePortal = "
							+ "Total conciergehourlyRideTripIDs = " + rideBookingIdsConcierge.size() + " - " + entry.getKey() + " - "
							+ entry.getValue());
