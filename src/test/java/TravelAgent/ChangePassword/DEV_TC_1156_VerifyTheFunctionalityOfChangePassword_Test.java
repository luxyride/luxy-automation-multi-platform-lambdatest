...
public class DEV_TC_1156_VerifyTheFunctionalityOfChangePassword_Test extends TestBase {
	...
	DEV_TC_1156_VerifyTheFunctionalityOfChangePassword_Main objVerifySighninMain;
	DEV_TC_1156_VerifyTheFunctionalityOfChangePassword_Test objVerifySighninTest;
	...
	public void VerifyLogInPageOfTravelAgentWithValidCredentials_Test() throws Exception {
		...
		try {
			...
			objVerifySighninMain = new DEV_TC_1156_VerifyTheFunctionalityOfChangePassword_Main(driver);
			objVerifySighninTest = new DEV_TC_1156_VerifyTheFunctionalityOfChangePassword_Test();
			objTestBase = new TestBase();
			...
			if (testStatus == "PASSED") {
				...
				objTestBase.defaultWaitTime(3000);
				testStep = "Verify page redirects to Homepage ";
				visibilityStatus = objVerifySighninMain.visibilityOfLoginDropdown(visibilityStatus);
				if (visibilityStatus.booleanValue() == true) {
					if (localExecutionFlag == true)	objupdateResults.updateResults(screenshotPath, logger, LogStatus.PASS, testStep, exception);	else	lambdaTestStatusUpdate("passed", testStep);
					testStatus = "PASSED";
				...
				objVerifySighninMain.clickChangePWDbutton();
				objTestBase.defaultWaitTime(2000);
				objVerifySighninMain.clickOnChangePWDSubmit();
				objTestBase.defaultWaitTime(3000);
				testStep = "Verify error message under text fields ";
				visibilityStatus = objVerifySighninMain.visibilityOfChangePwdBtn(visibilityStatus);
				if (visibilityStatus.booleanValue() == true) {
					if (localExecutionFlag == true)	objupdateResults.updateResults(screenshotPath, logger, LogStatus.PASS, testStep, exception);	else	lambdaTestStatusUpdate("passed", testStep);
					testStatus = "PASSED";
				} else {
