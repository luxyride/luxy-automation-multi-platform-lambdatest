...
public class DEV_TC_1542_VerifyAddressSuggestionListWhenSearchForHotelsMotelsInPickupOffExtraStopAddressFieldsTraveAgent_Test
		extends TestBase {
	...
	DEV_TC_1542_VerifyAddressSuggestionListWhenSearchForHotelsMotelsInPickupOffExtraStopAddressFieldsTraveAgent_Main objVerifySighninMain;
	DEV_TC_1542_VerifyAddressSuggestionListWhenSearchForHotelsMotelsInPickupOffExtraStopAddressFieldsTraveAgent_Test objVerifySighninTest;
	...
	public void VerifyLogInPageOfTravelAgentWithValidCredentials_Test() throws Exception {
		try {
			...
			objVerifySighninMain = new DEV_TC_1542_VerifyAddressSuggestionListWhenSearchForHotelsMotelsInPickupOffExtraStopAddressFieldsTraveAgent_Main(
					driver);
			objVerifySighninTest = new DEV_TC_1542_VerifyAddressSuggestionListWhenSearchForHotelsMotelsInPickupOffExtraStopAddressFieldsTraveAgent_Test();
			objTestBase = new TestBase();
	...
	public String vechileBooking(String scenario, String testStatus) throws Exception {
		try {
			...
			visibilityStatus = objVerifySighninMain.addFromAddress(visibilityStatus);
			...
			visibilityStatus = objVerifySighninMain.addToAddress(visibilityStatus);
			objTestBase.defaultWaitTime(2000);
			if (visibilityStatus.booleanValue() == true)
				if (localExecutionFlag == true)	objupdateResults.updateResults(screenshotPath, logger, LogStatus.PASS, testStep, exception);	else	browserStackTestStatusUpdate("passed", testStep);
			else
			...
			visibilityStatus = objVerifySighninMain.addExtraStop(visibilityStatus);
			objTestBase.defaultWaitTime(2000);
			if (visibilityStatus.booleanValue() == true)
				if (localExecutionFlag == true)	objupdateResults.updateResults(screenshotPath, logger, LogStatus.PASS, testStep, exception);	else	browserStackTestStatusUpdate("passed", testStep);
			else
